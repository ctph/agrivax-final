...
Clazz.load(["J.shapesurface.Isosurface"], "J.shapesurface.LcaoCartoon", ["JU.PT", "$.SB", "$.V3", "JU.C", "$.Escape"], function(){
...
Clazz.instantialize(this, arguments);}, J.shapesurface, "LcaoCartoon", J.shapesurface.Isosurface);
...
this.lcaoReverseColor = (value === Boolean.TRUE);
...
Clazz.defineMethod(c$, "setLcaoOn", 
function(iAtom, TF){
var id = this.getID(this.lcaoID, iAtom, false);
...
if (this.meshes[i].thisID.indexOf(id) >= 0) this.deleteMeshI(i);
}
}, "~N");
...
Clazz.defineMethod(c$, "createLcaoCartoon", 
function(iAtom){
var id = this.getID(this.lcaoID, iAtom, true);
...
for (var i = this.meshCount; --i >= 0; ) {
if (this.meshes[i].thisID.indexOf(id) == 0) {
this.meshes[i].setTranslucent(true, this.translucentLevel);
}}
}}, "~N");
Clazz.defineMethod(c$, "getID", 
function(id, i, withPrefix){
return (id != null ? id : (!withPrefix ? "_" : this.isLonePair || this.isRadical ? "lp_" : "lcao_") + (i + 1) + "_") + (this.thisType == null ? "" : JU.PT.rep(this.thisType, "-", (this.thisType.indexOf("-p") == 0 ? "" : "_")));
}, "~S,~N,~B");
