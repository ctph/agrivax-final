...
}, "javajs.api.GenericBinaryDocumentReader");
...
break;
case 32791:
this.writeBracketedGroupProperties(prop, len);
break;
case 32793:
this.writeCrossingBondProperties(prop, len);
break;
default:
this.skip(len);
break;
...
Clazz.defineMethod(c$, "writeBracketedGroupProperties", 
function(prop, len){
switch (prop) {
case 2599:
J.adapter.writers.CMLWriter.addAttribute(this.sb, "BracketedObjectIDs", this.readArray(len));
break;
case 2600:
J.adapter.writers.CMLWriter.addAttribute(this.sb, "RepeatCount", "" + Clazz.doubleToInt(this.readFloat64()));
break;
case 2596:
var usage = this.readInt(len);
var sval = null;
switch (usage) {
case 16:
sval = "MultipleGroup";
...
J.adapter.writers.CMLWriter.addAttribute(this.sb, "BracketUsage", sval);
break;
default:
this.skip(len);
}
}, "~N,~N");
Clazz.defineMethod(c$, "writeCrossingBondProperties", 
function(prop, len){
switch (prop) {
case 2604:
J.adapter.writers.CMLWriter.addAttribute(this.sb, "BondID", "" + this.readInt(len));
break;
case 2605:
J.adapter.writers.CMLWriter.addAttribute(this.sb, "InnerAtomID", "" + this.readInt(len));
break;
default:
this.skip(len);
}
...
break;
case 1073:
J.adapter.writers.CMLWriter.addAttribute(this.sb, "BondOrdering", this.readArray(len));
break;
case 1285:
J.adapter.writers.CMLWriter.addAttribute(this.sb, "ConnectionOrder", this.readArray(len));
break;
case 1074:
J.adapter.writers.CMLWriter.addAttribute(this.sb, "Attachments", this.readArray(-1));
break;
case 1075:
...
Clazz.defineMethod(c$, "readFloat64", 
function(){
return this.doc.readDouble();
});
...
function(len){
var n = (len < 0 ? this.doc.readShort() : Clazz.doubleToInt(len / 4));
var s = "";
for (var i = n; --i >= 0; ) {
s += " " + this.doc.readInt();
}
