...
if (isJSONMap && (readerName = J.adapter.smarter.Resolver.checkJsonHeader(leader)) != null) {
return readerName;
...
c$.checkJsonHeader = Clazz.defineMethod(c$, "checkJsonHeader", 
function(leader){
if (leader.startsWith("{\"SSG_international_symbol")) return "FSGOutput";
return null;
}, "~S");
c$.getBinaryType = Clazz.defineMethod(c$, "getBinaryType", 
function(inputStream){
var magic = JU.Rdr.getMagic(inputStream, 100);
var m0 = magic[0] & 0xff;
return (JU.Rdr.isPickleB(magic) ? "PyMOL" : m0 == 0xDE ? (J.adapter.smarter.Resolver.bytesMatch(magic, 4, J.adapter.smarter.Resolver.mmtfMagic) ? "MMTF" : null) : m0 == 0x83 ? (J.adapter.smarter.Resolver.checkBCIF(magic, true) ? "BCIF" : null) : J.adapter.smarter.Resolver.bytesMatch(magic, 0, J.adapter.smarter.Resolver.cdxMagic) ? "CDX" : J.adapter.smarter.Resolver.bytesMatch(magic, 0, J.adapter.smarter.Resolver.cmdfMagic) ? "Cmdf" : J.adapter.smarter.Resolver.bytesMatch(magic, 0, J.adapter.smarter.Resolver.cmdxMagic) ? "Cmdx" : null);
}, "java.io.InputStream");
c$.checkBCIF = Clazz.defineMethod(c$, "checkBCIF", 
function(magic, forCoord){
if (J.adapter.smarter.Resolver.bytesMatch(magic, 10, J.adapter.smarter.Resolver.bcifMolstarMagic) || J.adapter.smarter.Resolver.bytesMatch(magic, 10, J.adapter.smarter.Resolver.bcifModelMagic) || J.adapter.smarter.Resolver.bytesMatch(magic, 10, J.adapter.smarter.Resolver.bcifModelMagic) || J.adapter.smarter.Resolver.bytesMatch(magic, 10, J.adapter.smarter.Resolver.bcifCoordMagic)) return forCoord;
if (J.adapter.smarter.Resolver.bytesMatch(magic, 10, J.adapter.smarter.Resolver.bcifDensityMagic) || J.adapter.smarter.Resolver.bytesMatch(magic, 10, J.adapter.smarter.Resolver.bcifVolumeMagic)) return !forCoord;
return ((J.adapter.smarter.Resolver.findBytes(magic, J.adapter.smarter.Resolver.bcifServerMagic) < 0) == forCoord);
}, "~A,~B");
c$.findBytes = Clazz.defineMethod(c$, "findBytes", 
function(bytes, find){
out : for (var j = bytes.length - find.length; --j >= 0; ) {
for (var i = find.length; --i >= 0; ) {
if (bytes[j + i] != find[i]) continue out;
}
return j;
}
return -1;
...
c$.cdxMagic =  Clazz.newByteArray(-1, [86, 106, 67, 68]);
c$.cmdfMagic =  Clazz.newByteArray(-1, [67, 77, 68, 70]);
c$.cmdxMagic =  Clazz.newByteArray(-1, [67, 77, 68, 88]);
c$.mmtfMagic =  Clazz.newByteArray(-1, [109, 109, 116, 102]);
c$.bcifMolstarMagic =  Clazz.newByteArray(-1, [109, 111, 108, 42]);
c$.bcifDensityMagic =  Clazz.newByteArray(-1, [68, 101, 110, 115]);
c$.bcifVolumeMagic =  Clazz.newByteArray(-1, [86, 111, 108, 117]);
c$.bcifModelMagic =  Clazz.newByteArray(-1, [77, 111, 100, 101]);
c$.bcifCoordMagic =  Clazz.newByteArray(-1, [67, 111, 111, 114, 100]);
c$.bcifServerMagic =  Clazz.newByteArray(-1, [83, 69, 82, 86, 69, 82]);
