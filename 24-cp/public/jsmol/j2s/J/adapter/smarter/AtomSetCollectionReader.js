...
Clazz.load(["javajs.api.GenericLineReader", "JU.SB", "JV.Viewer"], "J.adapter.smarter.AtomSetCollectionReader", ["JU.BS", "$.Lst", "$.M3", "$.P3", "$.PT", "$.Quat", "$.V3", "J.adapter.smarter.Atom", "$.AtomSetCollection", "J.api.Interface", "$.JmolAdapter", "JU.BSUtil", "$.Logger", "$.SimpleUnitCell", "JV.FileManager", "$.JC"], function(){
...
Clazz.instantialize(this, arguments);}, J.adapter.smarter, "AtomSetCollectionReader", null, javajs.api.GenericLineReader);
Clazz.prepareFields (c$, function(){
this.next =  Clazz.newIntArray (1, 0);
this.highprecision0 = JV.Viewer.isHighPrecision;
this.loadNote =  new JU.SB();
});
Clazz.defineMethod(c$, "getPackingRangeValue", 
function(def){
...
Clazz.defineMethod(c$, "parsePrecision", 
function(s){
...
Clazz.defineMethod(c$, "setLowPrecision", 
function(){
...
this.unitCellParams =  Clazz.newFloatArray (27, 0);
...
if (isym < 0) JU.Logger.warn("Skipping symmetry operation " + xyz);
...
if (this.ucItems == null) this.ucItems =  Clazz.newFloatArray (6, 0);
this.ucItems[i] = x;
return;
...
Clazz.defineMethod(c$, "getFilterWithCase", 
function(key){
...
for (var i = this.moreUnitCellInfo.size(); --i >= 0; ) {
var s = this.moreUnitCellInfo.get(i).$plit("=");
this.asc.setCurrentModelInfo(s[0], s[1]);
}
});
...
Clazz.defineMethod(c$, "appendUnitCellInfo", 
function(info){
...
Clazz.defineMethod(c$, "fixFloatA", 
function(pts){
...
Clazz.defineMethod(c$, "fixFloatPt", 
function(pt, prec){
