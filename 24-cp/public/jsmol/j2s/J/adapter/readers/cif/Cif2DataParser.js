...
Clazz.load(["JU.CifDataParser"], "J.adapter.readers.cif.Cif2DataParser", ["java.util.Hashtable", "JU.PT"], function(){
var c$ = Clazz.declareType(J.adapter.readers.cif, "Cif2DataParser", JU.CifDataParser);
...
this.cterm = '}';
var ns = JU.CifDataParser.nullString;
JU.CifDataParser.nullString = null;
...
this.cterm = cterm0;
JU.CifDataParser.nullString = ns;
return (this.asObject ? map : "{" + str + "}");
});
Clazz.defineMethod(c$, "isSpaceOrColon", 
function(ich){
if (ich < this.cch) switch ((this.line.charAt(ich)).charCodeAt(0)) {
case 32:
case 9:
case 10:
case 58:
...
for (var i = 0; i < n; i++) a[i] = Clazz.floatToInt(f[i]);
