...
Clazz.load(["J.adapter.smarter.AtomSetCollectionReader", "JU.Lst", "$.P3"], "J.adapter.readers.cif.CifReader", ["java.util.Hashtable", "javajs.api.Interface", "JU.BS", "$.CifDataParser", "$.PT", "$.Rdr", "$.V3", "J.adapter.smarter.Atom", "J.api.JmolAdapter", "JU.Logger", "$.SimpleUnitCell", "$.Vibration"], function(){
...
this.addCellType("spin", this.parseUvwMath(this.field), false);
...
this.spinFrame = this.field;
}tag = "spinFrame";
break;
case "rotation_axis":
case "rotation_axis_xyz":
this.field = this.addSpinFrameExt("axisxyz");
break;
case "rotation_angle":
this.field = this.addSpinFrameExt("angle");
break;
case "collinear_direction":
this.field = this.addSpinFrameExt("coldir");
break;
...
this.appendUnitCellInfo(tag + "=" + this.field);
});
Clazz.defineMethod(c$, "addSpinFrameExt", 
function(name){
var val = JU.PT.replaceAllCharacters(this.field.toString(), "[]\"", "");
if (this.spinFrameExt == null) this.spinFrameExt = "";
this.spinFrameExt += ";" + name + "=" + val + ";";
return val;
}, "~S");
...
bs.setBitTo(i, this.asc.atoms[i].vib != null && this.asc.atoms[i].vib.lengthSquared() > 0);
}
...
var n = this.asc.getXSymmetry().setMagneticMoments();
...
this.setSpaceGroupName(this.lastSpaceGroupName = (this.key.indexOf("h-m") > 0 ? "HM:" : this.modulated ? "SSG:" : this.key.indexOf("spin") > 0 ? "spinSG:" : this.key.indexOf("bns") >= 0 ? "BNS:" : this.key.indexOf("hall") >= 0 ? "Hall:" : "") + s);
});
Clazz.defineMethod(c$, "addLatticeVectors", 
function(){
if (this.lstSpinLattices != null) {
if (this.asc.getSymmetry().addSpinLattice(this.lstSpinLattices)) this.appendLoadNote("Note! spin lattice added: " + this.lstSpinLattices);
this.lstSpinLattices = null;
return;
}this.lattvecs = null;
...
s += str + (m % n == 0 ? "=" : " ");
if (++m % n == 0) {
this.appendUnitCellInfo(s.trim());
s = "";
}}
} catch (e) {
if (Clazz.exceptionOf(e, Exception)){
...
ouvw = this.asc.getXSymmetry().getBaseSymmetry().transformUVW(ouvw, this.parseUvwMath(this.spinFrame), this.spinFrameExt);
field = oxyz + "(" + ouvw + ")";
timeRev = 0;
}
...
;//5.0.1-v7 Fri May 30 04:41:15 CDT 2025
