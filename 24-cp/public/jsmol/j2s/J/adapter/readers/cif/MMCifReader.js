...
Clazz.load(["J.adapter.readers.cif.CifReader"], "J.adapter.readers.cif.MMCifReader", ["java.util.Hashtable", "JU.BS", "$.Lst", "$.M4", "$.P3", "$.PT", "$.SB", "J.adapter.smarter.Atom", "$.Structure", "J.c.STR", "JU.BSUtil", "$.Logger"], function(){
...
this.modelStrings = "";
this.done = false;
Clazz.instantialize(this, arguments);}, J.adapter.readers.cif, "MMCifReader", J.adapter.readers.cif.CifReader);
...
if (this.isNull(g1 = this.getFieldString(1).toLowerCase()) || g1.length != 1 || this.isNull(g3 = this.getFieldString(0))) continue;
...
var m4 =  new JU.M4();
if (count != 12) {
this.symmetry.getRotTransArrayAndXYZ(xyz, m);
...
var hetName;
var groupType;
while (this.cifParser.getData()) if (!this.isNull(groupName = this.getFieldString(0)) && !this.isNull(hetName = this.getFieldString(1)) && !this.isNull(groupType = this.getFieldString(2))) this.addHetero(groupName, hetName, groupType, true, true);

return true;
});
Clazz.defineMethod(c$, "addHetero", 
function(groupName, hetName, groupType, doCheck, addNote){
...
if (this.carbohydrateGroups == null) this.carbohydrateGroups = "";
this.carbohydrateGroups += "[" + groupName + "]";
}if (addNote) this.appendLoadNote(groupName + " = " + hetName);
}, "~S,~S,~S,~B,~B");
...
var structure =  new J.adapter.smarter.Structure(-1, J.c.STR.HELIX, J.c.STR.HELIX, null, null, 0, null);
var type = this.getFieldString(0);
if (type.startsWith("TURN")) structure.structureType = structure.substructureType = J.c.STR.TURN;
 ...
 else structure.substructureType = J.adapter.smarter.Structure.getHelixType(this.parseIntFieldTok(9));
structure.strandID = this.getFieldString(8);
structure.structureID = this.getFieldString(7);
...
function(structure){
structure.startChainID = this.vwr.getChainID(this.getFieldString(1), true);
structure.startSequenceNumber = this.parseIntFieldTok(2);
structure.startInsertionCode = (this.getFieldString(3)).charAt(0);
structure.endChainID = this.vwr.getChainID(this.getFieldString(4), true);
structure.endSequenceNumber = this.parseIntFieldTok(5);
structure.endInsertionCode = (this.getFieldString(6)).charAt(0);
this.asc.addStructure(structure);
}, "J.adapter.smarter.Structure");
Clazz.defineMethod(c$, "parseIntFieldTok", 
function(tok){
return this.parseIntStr(this.getFieldString(tok));
}, "~N");
...
this.skipLoop(true);
return false;
}while (this.cifParser.getData()) this.addStructure( new J.adapter.smarter.Structure(-1, J.c.STR.SHEET, J.c.STR.SHEET, this.getFieldString(0), this.getFieldString(7), 1, null));
...
var resID;
while (this.cifParser.getData()) {
if (this.isNull(seqNum = this.getFieldString(3)) || this.isNull(resID = this.getFieldString(1))) continue;
var siteID = this.getFieldString(0);
htSite = this.htSites.get(siteID);
if (htSite == null) {
htSite =  new java.util.Hashtable();
htSite.put("groups", "");
this.htSites.put(siteID, htSite);
}var insCode = this.getFieldString(4);
var chainID = this.getFieldString(2);
...
while (this.cifParser.getData()) {
var sym1 = this.getFieldString(5);
var sym2 = this.getFieldString(11);
if (!sym1.equals(sym2) || !this.isNull(sym1) && !sym1.equals("1_555")) continue;
var type = this.getFieldString(12);
...
var key1 = "" + this.vwr.getChainID(this.getFieldString(0), true) + this.getFieldString(2) + this.parseIntFieldTok(1) + this.getFieldString(3) + this.getFieldString(4);
var key2 = "" + this.vwr.getChainID(this.getFieldString(6), true) + this.getFieldString(8) + this.parseIntFieldTok(7) + this.getFieldString(9) + this.getFieldString(10);
var order = this.getBondOrder(this.getFieldString(13));
...
while (this.cifParser.getData()) {
var comp = this.getFieldString(0);
var atom1 = this.getFieldString(1);
var atom2 = this.getFieldString(2);
var order = this.getBondOrder(this.getFieldString(3));
if (this.getFieldString(4).charAt(0) == 'Y') switch (order) {
case 1:
order = 513;
break;
case 2:
...
var modelNo = this.parseIntField();
...
if (this.carbohydrateGroups != null) {
this.asc.setInfo("carbohydrates", this.carbohydrateGroups);
this.asc.setCurrentModelInfo("carbohydrates", this.carbohydrateGroups);
}}});
c$.ncsoperFields =  Clazz.newArray(-1, ["*_matrix[1][1]", "*_matrix[1][2]", "*_matrix[1][3]", "*_vector[1]", "*_matrix[2][1]", "*_matrix[2][2]", "*_matrix[2][3]", "*_vector[2]", "*_matrix[3][1]", "*_matrix[3][2]", "*_matrix[3][3]", "*_vector[3]", "*_id", "*_symmetry_operation"]);
c$.operFields =  Clazz.newArray(-1, ["*_matrix[1][1]", "*_matrix[1][2]", "*_matrix[1][3]", "*_vector[1]", "*_matrix[2][1]", "*_matrix[2][2]", "*_matrix[2][3]", "*_vector[2]", "*_matrix[3][1]", "*_matrix[3][2]", "*_matrix[3][3]", "*_vector[3]", "*_id", "*_symmetry_operation"]);
c$.assemblyFields =  Clazz.newArray(-1, ["_pdbx_struct_assembly_gen_assembly_id", "_pdbx_struct_assembly_gen_oper_expression", "_pdbx_struct_assembly_gen_asym_id_list"]);
c$.structRefFields =  Clazz.newArray(-1, ["_struct_ref_seq_dif_mon_id", "_struct_ref_seq_dif_db_mon_id"]);
c$.chemCompFields =  Clazz.newArray(-1, ["_chem_comp_id", "_chem_comp_name", "_chem_comp_type"]);
c$.structConfFields =  Clazz.newArray(-1, ["*_conf_type_id", "*_beg_auth_asym_id", "*_beg_auth_seq_id", "*_pdbx_beg_pdb_ins_code", "*_end_auth_asym_id", "*_end_auth_seq_id", "*_pdbx_end_pdb_ins_code", "*_id", "*_pdbx_pdb_helix_id", "*_pdbx_pdb_helix_class"]);
c$.structSheetRangeFields =  Clazz.newArray(-1, ["*_sheet_id", "*_beg_auth_asym_id", "*_beg_auth_seq_id", "*_pdbx_beg_pdb_ins_code", "*_end_auth_asym_id", "*_end_auth_seq_id", "*_pdbx_end_pdb_ins_code", "*_id"]);
c$.structSiteFields =  Clazz.newArray(-1, ["*_site_id", "*_auth_comp_id", "*_auth_asym_id", "*_auth_seq_id", "*_pdbx_auth_ins_code"]);
