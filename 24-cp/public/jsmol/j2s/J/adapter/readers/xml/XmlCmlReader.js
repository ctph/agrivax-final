...
Clazz.load(["J.adapter.readers.xml.XmlReader"], "J.adapter.readers.xml.XmlCmlReader", ["java.util.Hashtable", "$.Properties", "$.StringTokenizer", "JU.BS", "$.Lst", "$.PT", "J.adapter.smarter.Atom", "$.Bond", "J.api.JmolAdapter", "JU.BSUtil", "$.Logger"], function(){
...
Clazz.instantialize(this, arguments);}, J.adapter.readers.xml, "XmlCmlReader", J.adapter.readers.xml.XmlReader);
...
if ((val = this.atts.get("name")) != null) this.thisAtom.atomName = val;
 else if ((val = this.atts.get("title")) != null) this.thisAtom.atomName = val;
 else if ((val = this.atts.get("label")) != null) this.thisAtom.atomName = val;
 else this.thisAtom.atomName = id;
if (!this.checkedSerial) {
this.isSerial = (id != null && id.length > 1 && id.startsWith("a") && JU.PT.parseInt(id.substring(1)) != -2147483648);
this.checkedSerial = true;
}if (this.isSerial) this.thisAtom.atomSerial = JU.PT.parseInt(id.substring(1));
...
this.thisAtom.set(this.parseFloatStr(val), this.parseFloatStr(this.atts.get("yfract")), this.parseFloatStr(this.atts.get("zfract")));
} else if ((val = this.atts.get("x3")) != null) {
this.thisAtom.set(this.parseFloatStr(val), this.parseFloatStr(this.atts.get("y3")), this.parseFloatStr(this.atts.get("z3")));
} else if ((val = this.atts.get("x2")) != null) {
this.thisAtom.set(this.parseFloatStr(val), this.parseFloatStr(this.atts.get("y2")), 0);
}if ((val = this.atts.get("elementtype")) != null) {
var sym = val;
if ((val = this.atts.get("isotope")) != null) this.thisAtom.elementNumber = ((this.parseIntStr(val) << 7) + J.api.JmolAdapter.getElementNumber(sym));
this.thisAtom.elementSymbol = sym;
}if ((val = this.atts.get("formalcharge")) != null) this.thisAtom.formalCharge = this.parseIntStr(val);
break;
}
break;
case 11:
...
this.thisAtom.partialCharge = this.parseFloatStr(this.chars.toString());
...
this.atomIdNames.put(this.thisAtom.atomName, this.chars.toString());
}}this.setKeepChars(false);
this.scalarTitle = null;
this.scalarDictRef = null;
break;
case 14:
this.state = 8;
if (this.scalarDictValue.equals("x3")) this.thisAtom.x = this.parseFloatStr(this.chars.toString());
 else if (this.scalarDictValue.equals("y3")) this.thisAtom.y = this.parseFloatStr(this.chars.toString());
 else if (this.scalarDictValue.equals("z3")) this.thisAtom.z = this.parseFloatStr(this.chars.toString());
 else if (this.scalarDictValue.equals("elementType")) this.thisAtom.elementSymbol = this.chars.toString();
this.setKeepChars(false);
break;
case 12:
var stereo = this.chars.toString();
if (this.thisBond.order == 1) this.thisBond.order = (stereo.equals("H") ? 1041 : 1025);
this.setKeepChars(false);
this.state = 11;
break;
case 15:
...
this.thisBond = this.asc.bonds[this.asc.bondCount - 1];
}}, "~S,~S,~N");
...
str = str.toUpperCase();
switch ((str.charAt(0)).charCodeAt(0)) {
case 49:
case 83:
return 1;
case 50:
...
c$.unitCellParamTags =  Clazz.newArray(-1, ["a", "b", "c", "alpha", "beta", "gamma"]);
});
