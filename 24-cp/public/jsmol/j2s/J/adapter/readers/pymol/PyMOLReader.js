...
Clazz.load(["J.adapter.readers.pdb.PdbReader", "JU.BS", "$.P3"], "J.adapter.readers.pymol.PyMOLReader", ["java.util.Hashtable", "JU.AU", "$.BC", "$.CU", "$.Lst", "$.PT", "$.V3", "J.adapter.readers.pymol.PickleReader", "$.PyMOL", "$.PyMOLScene", "J.adapter.smarter.Atom", "$.Bond", "$.Structure", "J.c.STR", "JU.BSUtil", "$.BoxInfo", "$.Logger"], function(){
...
this.aTemp = null;
this.ptTemp = null;
this.forceValence = false;
Clazz.instantialize(this, arguments);}, J.adapter.readers.pymol, "PyMOLReader", J.adapter.readers.pdb.PdbReader);
Clazz.prepareFields (c$, function(){
this.bsStructureDefined =  new JU.BS();
this.bsValence =  new JU.BS();
...
this.forceValence = this.checkFilterKey("SHOWVALENCE");
...
function(){
if (this.asc.atomSetCount == 1 && this.pymolScene.modelName != null && this.pymolScene.modelName.length == 4) this.pdbID = this.pymolScene.modelName;
Clazz.superCall(this, J.adapter.readers.pymol.PyMOLReader, "finalizeSubclassReader", []);
this.asc.setTensors();
});
...
this.pymolFrame = Clazz.doubleToInt(this.pymolScene.globalSetting(194));
this.getAtomAndStateCount(names);
this.pymolScene.setStateCount(this.stateCount);
var pymolState = Clazz.doubleToInt(this.pymolScene.globalSetting(193));
...
} else if (this.desiredModelNumber == -2147483648 && !this.allStates) {
this.addJmolScript("frame 1");
} else {
}var n = names.size();
var nAtomsJmol = 0;
for (var j = 0; j < this.stateCount; j++) {
for (var i = 1; i < n; i++) nAtomsJmol += this.processObject(J.adapter.readers.pymol.PyMOLReader.listAt(names, i), true, j, true);

}
this.pymolScene.setNumAtoms(nAtomsJmol);
for (var j = 0; j < this.stateCount; j++) {
...
this.trajectoryPtr = 0;
}for (var i = 1; i < n; i++) this.processObject(J.adapter.readers.pymol.PyMOLReader.listAt(names, i), true, j, false);

}
for (var i = 1; i < n; i++) this.processObject(J.adapter.readers.pymol.PyMOLReader.listAt(names, i), false, 0, false);
...
var i2 = J.adapter.readers.pymol.PyMOLReader.intAt(setting, 0);
if (i2 == -1) {
...
var type = J.adapter.readers.pymol.PyMOLReader.intAt(setting, 0);
var uid = (id * 1000) + type;
this.uniqueSettings.put(Integer.$valueOf(uid), setting);
n++;
if (JU.Logger.debugging) JU.Logger.info("PyMOL unique setting " + id + " " + setting);
}
}
}JU.Logger.info("PyMOL uniqueSettings: " + n);
return max;
}, "JU.Lst");
...
return (this.objectName != null && this.objectName.indexOf("_") != 0);
}, "JU.Lst");
...
function(execObject, moleculeOnly, iState, justCountJmolAtoms){
if (execObject == null) return 0;
var type = J.adapter.readers.pymol.PyMOLReader.intAt(execObject, 4);
var startLen = execObject.get(execObject.size() - 1);
if ((type == 1) != moleculeOnly || !this.checkObject(execObject)) return 0;
var pymolObject = J.adapter.readers.pymol.PyMOLReader.listAt(execObject, 5);
...
var idxToAtm = J.adapter.readers.pymol.PyMOLReader.listAt(state, 3);
if (iState > 0 && (idxToAtm == null || idxToAtm.size() == 0)) return 0;
if (justCountJmolAtoms) {
return J.adapter.readers.pymol.PyMOLReader.intAt(pymolObject, 3);
}stateSettings = J.adapter.readers.pymol.PyMOLReader.listAt(state, 7);
} else if (iState > 0) {
return 0;
}JU.Logger.info("PyMOL model " + (this.nModels) + " state " + iState + " Object " + this.objectName + (this.isHidden ? " (hidden)" : " (visible)"));
...
var bsReps = J.adapter.readers.pymol.PyMOLReader.getBsReps(-1, setting, 3);
...
var coords;
var labelPositions;
var uniqueIds = J.adapter.readers.pymol.PyMOLReader.listAt(state, 11);
var idxArray = null;
...
coordsArray =  Clazz.newDoubleArray (nAtoms * 3, 0);
this.fillDoubleArrayFromBytes(state.get(2), coordsArray);
this.fillIntArrayFromBytes(state.get(3), idxArray);
var b = state.get(8);
if (b != null) {
labelArray =  Clazz.newDoubleArray (nAtoms * 7, 0);
this.fillDoubleArrayFromBytes(b, labelArray);
}} else {
...
this.pymolToAdapterAtom =  Clazz.newIntArray (nAtoms, 0);
var bsAtoms = this.pymolScene.setAtomMap(this.pymolToAdapterAtom, this.ac0);
for (var i = 0; i < 23; i++) this.reps[i] = JU.BS.newN(1000);

this.pymolScene.htSpacefill.clear();
...
var index = (idxToAtm != null ? J.adapter.readers.pymol.PyMOLReader.intAt(idxToAtm, idx) : idxArray != null ? idxArray[idx] : idx);
var a = this.addAtom(pymolAtoms, index, atomArray, vArray, lexStr, idx, coords, coordsArray, labelPositions, uniqueIds, labelArray, bsAtoms, iState);
if (a != null) this.trajectoryStep[this.trajectoryPtr++] = a;
}
}this.addBonds(this.getBondList(J.adapter.readers.pymol.PyMOLReader.listAt(pymolObject, 6)));
this.addMolStructures();
this.atoms = this.asc.atoms;
if (!this.isStateScript) {
this.asc.setCurrentModelInfo("name", this.pymolScene.modelName);
...
var isSpecial = (order > 1 && this.bsValence.get(ia) && this.bsValence.get(ib));
var isSingle = (order == 1 || asSingle && !isSpecial);
order |= (isSingle ? 65536 : this.pymolScene.encodeMultipleBond(uid, isSpecial));
var bond =  new J.adapter.smarter.Bond(ia, ib, order);
bond.uniqueID = uid;
bondList.addLast(bond);
}
this.bsValence.clearAll();
return bondList;
}, "JU.Lst");
...
Clazz.defineMethod(c$, "fillDoubleArrayFromBytes", 
function(b, array){
try {
...
function(pymolAtoms, apt, atomArray, vArray, lexStr, idx, coords, coordArray, labelPositions, uniqueIds, labelArray, bsState, iState){
this.pymolToAdapterAtom[apt] = -1;
var chainID;
var altLoc;
var group3;
var name;
...
var anisou = null;
var bsReps = null;
var id = (uniqueIds == null ? null : uniqueIds.get(idx));
if (id != null) {
...
if (uniqueID == -1) uniqueID = this.atomInt(atomArray, pt, vArray[13]);
...
bsReps = J.adapter.readers.pymol.PyMOLReader.getBsReps(apt, a, 20);
...
if (uniqueID == -1) uniqueID = (a.size() > 40 && J.adapter.readers.pymol.PyMOLReader.intAt(a, 40) == 1 ? J.adapter.readers.pymol.PyMOLReader.intAt(a, 32) : -1);
if (a.size() > 46) anisou = J.adapter.readers.pymol.PyMOLReader.floatsAt(a, 41,  Clazz.newFloatArray (8, 0), 6);
}var v = this.pymolScene.getUniqueFloatDef(uniqueID, 64);
if (this.forceValence || v == 1) {
this.bsValence.set(apt);
}if (insCode == null) {
var len = resi.length;
...
if (this.pymolScene.getUniqueFloatDef(uniqueID, 64) == 1) {
this.bsValence.set(apt);
...
}}labelPos = J.adapter.readers.pymol.PyMOL.fixAllZeroLabelPosition(labelPos);
...
this.pymolScene.jmolToUniqueID[this.$ac] = uniqueID;
this.pymolToAdapterAtom[apt] = this.$ac++;
return null;
}, "JU.Lst,~N,~A,~A,~A,~N,JU.Lst,~A,JU.Lst,JU.Lst,~A,JU.BS,~N");
...
var a1 = this.pymolToAdapterAtom[bond.atomIndex1];
var a2 = this.pymolToAdapterAtom[bond.atomIndex2];
if (a1 < 0 || a2 < 0) continue;
bond.atomIndex1 = a1;
bond.atomIndex2 = a2;
bond.pymolRadius = this.pymolScene.getRadiusForBond(bond.uniqueID, a1, a2);
if (bond.uniqueID != -1) {
...
var radii =  Clazz.newDoubleArray (this.$ac, 0);
...
function(list, i){
var o = (list == null ? null : list.get(i));
...
function(apt, setting, pt){
if (setting.size() <= pt) return null;
var bsReps =  new JU.BS();
var o = setting.get(pt);
if (Clazz.instanceOf(o, Integer)) {
var bits = (o).intValue();
for (var i = 0, b = 1; i < 32; i++) {
if ((bits & b) != 0) {
bsReps.set(i);
}b <<= 1;
}
...
Clazz.defineMethod(c$, "getCartoonType", 
function(iAtom){
return Clazz.floatToInt(this.atoms[iAtom].vib.y);
}, "~N");
Clazz.defineMethod(c$, "getVDW", 
function(iAtom){
...
Clazz.defineMethod(c$, "compareAtoms", 
function(iPrev, i){
