...
Clazz.load(null, "J.adapter.readers.pymol.JmolObject", ["java.util.Hashtable", "JU.P3", "$.PT", "$.SB", "J.adapter.readers.pymol.PyMOLReader", "JU.BSUtil", "$.Escape"], function(){
...
this.rd = null;
this.cacheID = null;
Clazz.instantialize(this, arguments);}, J.adapter.readers.pymol, "JmolObject", null);
Clazz.makeConstructor(c$, 
function(shape, branchNameID, bsAtoms, info){
this.shape = shape;
...
}sm.vwr.setObjectProp(this.info, this.shape);
} else {
...
var bsBonds = sm.vwr.getBondsForSelectedAtoms(this.bsAtoms);
if (this.info != null) {
var o = this.info;
var bsH = o[0];
var stick_h_scale = (o[1]).floatValue();
var hsize = Clazz.floatToInt(this.size * stick_h_scale);
...
case 16:
sm.loadShape(this.shape);
sm.setShapePropertyBs(this.shape, "ignore", JU.BSUtil.copyInvert(this.bsAtoms, sm.vwr.ms.ac), null);
break;
default:
...
map.put("mesh_width", Double.$valueOf(pymolScene.getDefaultFloat(90)));
...
sb.append(" model ").append(model).append(resolution).append(" select ").append(JU.Escape.eBS(this.bsAtoms)).append(only).append(ignore).append(type).appendD(Math.abs(this.size / 1000));
if (!haveMep) {
...
return;
}if (this.size != -1 || this.rd != null) sm.setShapeSizeBs(this.shape, this.size, this.rd, this.bsAtoms);
if (this.argb != 0) sm.setShapePropertyBs(this.shape, color, Integer.$valueOf(this.argb), this.bsAtoms);
if (this.translucency > 0) {
sm.setShapePropertyBs(this.shape, "translucentLevel", Double.$valueOf(this.translucency), this.bsAtoms);
sm.setShapePropertyBs(this.shape, "translucency", "translucent", this.bsAtoms);
} else if (this.colors != null) sm.setShapePropertyBs(this.shape, "colors", this.colors, this.bsAtoms);
...
if (iAtom >= m.at.length) System.err.println("JmolObject PyMOL LOADING ERROR IN MERGE. atom index too large");
return (iAtom < 0 ? -1 : m.at[iAtom].mi);
}, "JM.ModelSet");
Clazz.defineMethod(c$, "setColors", 
function(colixes, translucency){
this.colors =  Clazz.newArray(-1, [colixes, Double.$valueOf(translucency)]);
}, "~A,~N");
Clazz.defineMethod(c$, "setSize", 
function(size){
this.size = Clazz.doubleToInt(size * 1000);
}, "~N");
