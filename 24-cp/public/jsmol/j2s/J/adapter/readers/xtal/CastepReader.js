...
Clazz.load(["J.adapter.smarter.AtomSetCollectionReader"], "J.adapter.readers.xtal.CastepReader", ["JU.DF", "$.Lst", "$.M3", "$.M4", "$.P3", "$.PT", "$.V3", "J.adapter.smarter.Atom", "JS.SymmetryOperation", "JU.Logger", "$.SimpleUnitCell", "$.Tensor"], function(){
...
this.tsType = null;
this.allowSymmetryGeneration = true;
this.lattTr = null;
this.matSupercell = null;
Clazz.instantialize(this, arguments);}, J.adapter.readers.xtal, "CastepReader", J.adapter.smarter.AtomSetCollectionReader);
...
continue;
}if (this.tokens[1].equalsIgnoreCase("SYMMETRY_OPS")) {
this.readSymmetryOps();
continue;
}} else if (this.allowSymmetryGeneration && this.tokens[0].equalsIgnoreCase("SYMMETRY_GENERATE")) {
...
Clazz.defineMethod(c$, "readSymmetryOps", 
function(){
if (this.lattTr == null) {
this.setLatticeVectors();
var uc = this.getSymmetry().setUnitCellFromParams(this.unitCellParams, false, 0.0001);
var vecs = uc.getUnitCellVectors();
this.lattTr =  new JU.M3();
this.lattTr.setColumnV(0, vecs[1]);
this.lattTr.setColumnV(1, vecs[2]);
this.lattTr.setColumnV(2, vecs[3]);
}var lattTrInv = JU.M3.newM3(this.lattTr);
lattTrInv.invert();
var op =  new JU.M4();
var nop = 0;
...
var x = this.parseCalcStr(this.tokens[0]);
var y = this.parseCalcStr(this.tokens[1]);
var z = this.parseCalcStr(this.tokens[2]);
if (i < 3) {
op3.setColumn3(i, x, y, z);
} else {
t.set(x, y, z);
}} else {
JU.Logger.warn("cannot read line with CASTEP symmetr_ops: " + this.line);
}this.tokenizeCastepCell();
}
op3.mul2(op3, this.lattTr);
op3.mul2(lattTrInv, op3);
op.setMV(op3, t);
var xyz = JS.SymmetryOperation.getXYZFromMatrix(op, false, true, false);
nop++;
JU.Logger.info("CASTEP reader op[" + nop + "] = " + xyz);
this.setSymmetryOperator(xyz);
}
});
Clazz.defineMethod(c$, "parseCalcStr", 
function(s){
var f = JU.SimpleUnitCell.parseCalcFunctions(this.vwr, null, s);
if (Float.isNaN(f)) {
JU.Logger.error("CASTEP math error in " + s);
}return f;
}, "~S");
...
for (var i = 0; i < 3; i++) {
if (this.tokens.length >= 3) {
x = this.parseCalcStr(this.tokens[0]) * factor;
y = this.parseCalcStr(this.tokens[1]) * factor;
z = this.parseCalcStr(this.tokens[2]) * factor;
...
JU.Logger.info("tensor " + atom.atomName + " " + line0);
var a = this.fill3x3(JU.PT.getTokens(line0), 0);
