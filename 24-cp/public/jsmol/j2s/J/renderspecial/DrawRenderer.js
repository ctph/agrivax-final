...
this.s2f = null;
this.bsHandles = null;
this.haveNotifiedHandles = false;
Clazz.instantialize(this, arguments);}, J.renderspecial, "DrawRenderer", J.render.MeshRenderer);
...
if (!this.isExport && mesh.visibilityFlags != 0 && (this.vwr.getDrawHover() || this.vwr.getPickingMode() == 4)) {
...
}if (this.dmesh.haveXyPoints && this.drawXYPoints()) return;
var tension = 5;
...
this.renderArrowWithHead(this.pt0, this.pt1, 0.3, false, false, this.dmesh.isBarb);
...
if (!isCurved) {
this.renderArrowWithHead(this.vertices[0], this.vertices[1], 0, false, true, this.dmesh.isBarb);
return;
}var nHermites = 5;
...
this.renderArrowWithHead(this.controlHermites[nHermites - 2], this.controlHermites[nHermites - 1], 0, false, false, this.dmesh.isBarb);
break;
}
...
Clazz.defineMethod(c$, "drawXYPoints", 
function(){
if (this.dmesh.isVector) {
var ptXY = 0;
for (var i = 0; i < 2; i++) if (J.shapespecial.Draw.is2DPoint(this.vertices[i])) ptXY += i + 1;

if (--ptXY < 2) {
this.renderXyArrow(ptXY);
return true;
}} else if (this.drawType === J.shapespecial.Draw.EnumDrawType.POINT || this.drawType === J.shapespecial.Draw.EnumDrawType.MULTIPLE) {
this.renderXyPoint();
return true;
}return false;
});
...
this.renderArrowWithHead(this.pt0, this.pt1, 0, true, false, false);
}, "~N");
Clazz.defineMethod(c$, "renderArrowWithHead", 
function(pt1, pt2, factor2, isTransformed, withShaft, isBarb){
if (this.dmesh.noHead && !withShaft) return;
if (this.s0f == null) {
this.s0f =  new JU.P3();
this.s1f =  new JU.P3();
this.s2f =  new JU.P3();
...
if (d == 0) return;
var headWidth = (this.width > 0 && !this.dmesh.noHead ? this.width * 3 : 0);
...
if (len > headWidth * 1.5) this.vTemp.scale(headWidth * 1.5 / len);
...
if (headDiameter > 2 && !this.dmesh.noHead) this.g3d.fillConeScreen3f(2, headDiameter, this.s1f, this.s2f, isBarb);
...
if (k >= this.screens.length) {
if (!this.haveNotifiedHandles) {
