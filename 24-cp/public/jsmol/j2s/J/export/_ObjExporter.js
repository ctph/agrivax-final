...
if (meshSurface.offset != null) matrix.setTranslation(JU.V3.newV(meshSurface.offset));
...
sb.append(" Ns 163\n");
sb.append(" Tr " + J["export"].___Exporter.translucencyFractionalFromColix(colix) + "\n");
sb.append(" Ni 0.001\n");
sb.append(" illum 2\n");
...
for (var i = 0; i < nFaces; i++) if (dim != null) this.outputFace2(faces[i], i, map, map2);
 else this.outputFace1(faces[i], map, map2, normals != null);
...
function(face, map, map2, withNormals){
this.output("f");
for (var i, $i = 0, $$i = face; $i < $$i.length && ((i = $$i[$i]) || true); $i++) this.output(" " + ((map == null ? i : map[i]) + this.currentVertexOrigin) + (withNormals ? "//" + ((map2 == null ? i : map2[i]) + this.currentNormalOrigin) : ""));
