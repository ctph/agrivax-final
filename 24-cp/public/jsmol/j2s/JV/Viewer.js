...
Clazz.instantialize(this, arguments);}, JV, "Viewer", J.api.JmolViewer, [J.atomdata.AtomDataServer, J.api.PlatformViewer]);
...
Clazz.defineMethod(c$, "isSubstructure", 
...
Clazz.defineMethod(c$, "hasStructure", 
function(pattern, smiles, isSmarts){
try {
var ret = this.getSmilesMatcher().hasStructure(pattern,  Clazz.newArray(-1, [smiles]), (isSmarts ? 2 : 1) | 8);
return ret[0] == 1;
} catch (e) {
if (Clazz.exceptionOf(e, Exception)){
return false;
...
this.access = this.access0 = (this.checkOption2("access:READSPT", "-r") ? JV.Viewer.ACCESS.READSPT : this.checkOption2("access:NONE", "-R") ? JV.Viewer.ACCESS.NONE : JV.Viewer.ACCESS.ALL);
...
if (JV.Viewer.isJS && this.haveDisplay) this.acm.createActions();
} else {
...
if (this.isSilent) JU.Logger.setLogLevel(3);
 else JU.Logger.info("Jmol version " + JV.Viewer.getJmolVersion());
...
}}this.useCommandThread = !JV.Viewer.isJS && !this.headless && this.checkOption2("useCommandThread", "-threaded");
...
function(isStereo, isImageWrite){
this.gdata.beginRendering(this.tm.getStereoRotationMatrix(isStereo), this.g.translucent, isImageWrite, !this.checkMotionRendering(1111490588));
}, "~B,~B");
...
this.modelkit = J.api.Interface.getInterface("J.modelkit.ModelKit", this, "script");
if (this.headless || !this.haveDisplay) {
this.modelkit.setProperty("headless", this);
} else {
this.modelkit.setMenu(this.apiPlatform.getMenuPopup(null, 'm'));
}} else if (andShow) {
...
Clazz.defineMethod(c$, "showSystemInfo", 
function(){
if (!this.isSilent && !this.systemInfoShown) {
...
Clazz.defineMethod(c$, "readFileAsMap", 
...
this.setNewRotationCenter(this.getUnitCellCenter());
} else {
...
if (type == '\0') this.tm.translateToPercent('x', x);
 else this.tm.translateXYBy(Clazz.floatToInt(xy), 0);
break;
case 89:
case 121:
if (type == '\0') this.tm.translateToPercent('y', x);
 else this.tm.translateXYBy(0, Clazz.floatToInt(xy));
break;
case 90:
case 122:
if (type == '\0') this.tm.translateToPercent('z', x);
 else this.tm.translateZBy(Clazz.floatToInt(xy));
break;
}
...
if (!strModel.startsWith(JV.JC.BASE64_TAG)) {
...
Clazz.defineMethod(c$, "loadDefaultModelKitModel", 
function(htParams){
...
function(){
var sym = this.getUnitCell(this.am.cmi);
if (sym == null) return null;
var offset = sym.getCartesianOffset();
var iAtom = this.am.getUnitCellAtomIndex();
return (iAtom >= 0 && (offset == null || offset.length() == 0) ? this.ms.getUnitCellForAtom(iAtom) : sym);
});
...
Clazz.defineMethod(c$, "toCartesianUC", 
...
Clazz.defineMethod(c$, "toFractionalUC", 
function(unitCell, pt, ignoreOffset){
if (unitCell == null) unitCell = this.getCurrentUnitCell();
if (unitCell != null) {
...
var keys = null;
var pt = (fname == null ? -1 : fname.indexOf(";keys="));
if (pt >= 0) {
keys = fname.substring(pt + 6).$plit(",");
...
}var data = (fname == null || fname.length == 0 ? this.getCurrentFileAsString("script") : this.getFileAsString3(fname, false, null));
if (data == null || data.length < 2) return null;
var rdr = JU.Rdr.getBR(data);
if (type == null) type = this.getModelAdapter().getFileTypeName(rdr);
return (type == null ? null : this.readCifDataKey(null, rdr, type, keys));
}, "~S,~S");
Clazz.defineMethod(c$, "readCifData", 
function(fileName, rdrOrStringData, type){
return this.readCifDataKey(fileName, rdrOrStringData, type, Clazz.castNullAs("Array"));
}, "~S,~O,~S");
Clazz.defineMethod(c$, "readCifDataKey", 
function(fileName, rdrOrStringData, type, keys){
...
return JU.Rdr.readCifData(J.api.Interface.getInterface(("Cif2".equals(type) ? "J.adapter.readers.cif.Cif2DataParser" : "JU.CifDataParser"), this, "script"), rdr, keys);
}, "~S,~O,~S,~A");
...
}}if (this.isCapturing()) this.doCapture();
this.notifyViewerRepaintDone();
}, "~O,~B,~N,~N");
Clazz.defineMethod(c$, "isCapturing", 
function(){
return (this.captureParams != null && Boolean.FALSE !== this.captureParams.get("captureEnabled"));
});
Clazz.defineMethod(c$, "doCapture", 
function(){
...
}if (id.indexOf("_") < 0) id = this.getSymTemp().getSpaceGroupJSON("AFLOWLIB", id, index);
...
case 73:
case 84:
format = this.g.nihResolverFormat + "/%FILE/file?format=stdinchi";
break;
case 75:
case 83:
...
Clazz.defineMethod(c$, "isModelkitPickingActive", 
function(){
...
Clazz.defineMethod(c$, "isModelkitPickingRotateBond", 
function(){
...
Clazz.defineMethod(c$, "isModelKitOpen", 
function(){
...
break;
case 603979870:
this.g.labelKey = value;
if (value) this.g.elementKey = false;
this.getModelkit(false).setProperty("setlabelkey", Boolean.$valueOf(value));
break;
case 603979838:
this.g.elementKey = value;
if (value) this.g.labelKey = false;
...
function(eval, point1, point2, degreesPerSecond, endDegrees, isSpin, bsSelected, translation, finalPoints, dihedralList, vectorMatrix, m4, useModelKit, centerAndPoints){
if (eval == null) eval = this.eval;
if (this.headless) {
if (isSpin && endDegrees == 3.4028235E38) return false;
isSpin = false;
}var isOK = this.tm.rotateAboutPointsInternal(eval, point1, point2, degreesPerSecond, endDegrees, false, isSpin, bsSelected, false, translation, finalPoints, dihedralList, vectorMatrix, m4, useModelKit, centerAndPoints);
if (isOK) this.setSync();
return isOK;
}, "J.api.JmolScriptEvaluator,JU.P3,JU.P3,~N,~N,~B,JU.BS,JU.V3,JU.Lst,~A,JU.M3,JU.M4,~B,~A");
...
return;
}this.tm.rotateAboutPointsInternal(null, pt1, pt2, this.g.pickingSpinRate, 3.4028235E38, isClockwise, true, null, false, null, null, null, null, null, false, null);
}, "JU.T3,JU.T3,~B");
...
Clazz.defineMethod(c$, "invertAtomCoord", 
...
Clazz.defineMethod(c$, "moveSelectedXY", 
...
if (deltaZ != -2147483648) {
if (uc != null) return 0;
this.ptScreenNew.set(this.ptScreen.x, this.ptScreen.y, this.ptScreen.z + deltaZ);
} else {
this.ptScreenNew.set(this.ptScreen.x + deltaX * f, this.ptScreen.y + deltaY * f, this.ptScreen.z);
}this.tm.unTransformPoint(this.ptScreenNew, this.ptNew);
ptNew = this.ptNew;
if (uc != null) {
ptNew.sub(ptCenter);
ptNew.add(this.ms.at[iatom]);
this.getModelkit(false).assignMoveAtoms(bsSelected, iatom, ptNew, null, true, !asAtoms, false);
...
Clazz.defineMethod(c$, "warnAtom", 
function(iatom){
if (iatom < 0 && this.atomHighlighted < 0) return;
this.atomHighlighted = iatom;
this.shm.loadShape(8);
this.setShapeProperty(8, "warnAtom", (iatom >= 0 ? Integer.$valueOf(iatom) : null));
}, "~N");
...
case 75:
case 77:
if (s.startsWith("InChIKey=")) s = s.substring(9);
if (s.indexOf("\n") > 0) s = s.substring(0, s.indexOf("\n"));
break;
default:
if (info.equals("jme")) s = this.getPropertyManager().fixJMEFormalCharges(bsAtoms, s);
break;
}
return s;
}, "~S,~S,JU.BS");
...
Clazz.defineMethod(c$, "writeFile", 
...
function(bsDeleted){
var n = this.ms.bondCount;
for (var i = bsDeleted.nextSetBit(0); i >= 0; i = bsDeleted.nextSetBit(i + 1)) {
if (i >= n || this.ms.bo[i] == null) bsDeleted.clear(i);
}
var bi = bsDeleted.nextSetBit(0);
if (bi < 0) return;
var modelIndex = this.ms.bo[bi].atom1.mi;
...
Clazz.defineMethod(c$, "setMotionFixed", 
function(bs){
this.slm.setMotionFixedAtoms(bs);
}, "JU.BS");
...
Clazz.defineMethod(c$, "isModel2D", 
function(bs){
...
function(script){
return "" + this.evaluateExpression( Clazz.newArray(-1, [ Clazz.newArray(-1, [JS.T.tokenScript, JS.T.tokenLeftParen, JS.SV.newS(script), JS.T.tokenRightParen])]));
}, "~S");
...
Clazz.overrideMethod(c$, "evaluateExpressionAsVariable", 
function(stringOrTokens){
...
this.shm.setShapePropertyBs(23, "setCGO", map, null);
}, "java.util.Map");
...
var me = this;
var r = ((Clazz.isClassDefined("JV.Viewer$1") ? 0 : JV.Viewer.$Viewer$1$ ()), Clazz.innerTypeInstance(JV.Viewer$1, this, Clazz.cloneFinals("me", me, "iAtom", iAtom)));
{
...
Clazz.defineMethod(c$, "getModelSetAuxiliaryInfoForAtoms", 
function(atomExpression){
...
function(){
return J.api.Interface.getSymmetry(this, "ms").setViewer(this);
});
Clazz.defineMethod(c$, "getSymStatic", 
function(){
return (this.symStatic == null ? (this.symStatic = J.api.Interface.getSymmetry(this, "ms")).setViewer(this) : this.symStatic);
});
...
Clazz.defineMethod(c$, "getModelkitPropertySafely", 
function(name){
return (this.modelkit == null ? null : name == null ? Boolean.TRUE : this.modelkit.getProperty(name));
}, "~S");
Clazz.defineMethod(c$, "setModelkitPropertySafely", 
function(key, value){
return (this.modelkit == null ? null : this.modelkit.setProperty(key, value));
}, "~S,~O");
Clazz.defineMethod(c$, "isModelKitOption", 
function(type, value){
...
Clazz.defineMethod(c$, "getConsoleFontScale", 
function(){
return this.consoleFontScale;
});
Clazz.defineMethod(c$, "setConsoleFontScale", 
function(scale){
...
Clazz.defineMethod(c$, "evalStringGUI", 
function(script){
this.evalStringQuiet(script + "; ## GUI ##");
}, "~S");
Clazz.defineMethod(c$, "selectStatus", 
...
Clazz.defineMethod(c$, "wasmInchiHack", 
function(cmd){
...
Clazz.defineMethod(c$, "restrictToModel", 
function(bs, mi){
if (bs == null) bs = this.bsA();
...
Clazz.defineMethod(c$, "getThisModelAtoms", 
function(){
...
Clazz.defineMethod(c$, "getOperativeSymmetry", 
function(){
...
Clazz.defineMethod(c$, "getScaleText", 
...
m = Clazz.doubleToInt(Math.floor(Math.log10(e)));
mp = Math.pow(10, m);
e = JM.Measurement.fromUnits(mp + 0.000001, u);
p = Clazz.floatToInt(e * this.tm.scalePixelsPerAngstrom / af);
if (p < min) {
d *= 10;
}}
var se = (m >= 0 ? " " + Clazz.floatToInt(mp) + " " : " 0." + "000000000".substring(0, -1 - m) + "1 ");
ret[0] = p;
ret[1] = e;
return se + u;
}, "~S,~B,~N,~A");
...
Clazz.defineMethod(c$, "getFormulaForAtoms", 
...
Clazz.defineMethod(c$, "haveAccessInternal", 
function(path){
if (path == null) {
return this.access === JV.Viewer.ACCESS.INTERNAL;
}if (JV.Viewer.isJS) return true;
path = path.$replace('\\', '/');
var ret = this.access !== JV.Viewer.ACCESS.NONE && (this.internalAccessPath == null || this.access !== JV.Viewer.ACCESS.INTERNAL || path.startsWith(this.internalAccessPath));
if (!ret) System.err.println("Viewer: Internal file access in " + this.internalAccessPath + " denied for " + path);
return ret;
}, "~S");
Clazz.defineMethod(c$, "setAccessInternal", 
function(path){
if (JV.Viewer.isJS) return;
if (path == null) {
this.access = this.access0;
} else if (this.access !== JV.Viewer.ACCESS.INTERNAL) {
this.access = JV.Viewer.ACCESS.INTERNAL;
}this.internalAccessPath = path;
}, "~S");
Clazz.defineMethod(c$, "getItaNumberFor", 
function(name){
var s = this.getSymStatic().getSpaceGroupInfoObj("itaNumber", name, false, false);
return (s == null ? -1 : JU.PT.parseInt(s));
}, "~S");
Clazz.defineMethod(c$, "is2D", 
function(){
return this.getBoolean(603979882);
});
Clazz.defineMethod(c$, "getUnitCellCenter", 
function(){
var uc = this.getCurrentUnitCell();
return (uc == null ?  new JU.P3() : uc.getUnitCellCenter());
});
Clazz.defineMethod(c$, "setUnitCellAtomIndex", 
function(iAtom){
var sym = this.getCurrentUnitCell();
if (sym != null) sym.setOffsetPt( new JU.P3());
this.am.setUnitCellAtomIndex(iAtom);
}, "~N");
Clazz.defineMethod(c$, "getUnitCellAtomIndex", 
function(){
return this.am.getUnitCellAtomIndex();
});
Clazz.defineMethod(c$, "writeBinaryFile", 
...
function(modelIndex, rot){
if (modelIndex < 0) modelIndex = this.am.cmi;
if (modelIndex < 0 || modelIndex >= this.ms.mc) return;
if (rot == null) this.tm.setSpinOff();
this.ms.rotateModelSpinVectors(modelIndex, rot);
}, "~N,JU.M3");
c$.$Viewer$1$=function(){
/*if5*/;(function(){
