...
Clazz.load(["JU.A4", "$.M3", "$.M4", "$.P3", "$.P3i", "$.V3", "J.c.STER", "JU.Point3fi"], "JV.TransformManager", ["java.util.Hashtable", "JU.P4", "$.Quat", "$.SB", "J.api.Interface", "JU.Escape", "$.Logger"], function(){
...
this.nav = null;
Clazz.instantialize(this, arguments);}, JV, "TransformManager", null);
...
this.rotateAboutPointsInternal(null, pt2, pt1, 10 * speed, NaN, false, true, null, true, null, null, null, null, null, false, null);
}, "~N,~N,~N");
Clazz.defineMethod(c$, "rotateXYBy", 
function(degX, degY, bsAtoms){
if (this.vwr.is2D()) return;
...
if (this.matrixTemp3.setAsBallRotation(0.017453292519943295, -yDeg, -xDeg)) this.applyRotation(this.matrixTemp3, false, bsAtoms, null, false, null, true);
}, "~N,~N,JU.BS");
...
this.setSpin(eval, true, endDegrees, null, null, null, bsAtoms, false, null, -1);
...
function(eval, point1, point2, degreesPerSecond, endDegrees, isClockwise, isSpin, bsAtoms, isGesture, translation, finalPoints, dihedralList, vectorMatrix, m4, useModelKit, centerAndPoints){
if (isSpin) this.setSpinOff();
this.setNavOn(false);
if (vectorMatrix == null && dihedralList == null && centerAndPoints == null && (translation == null || translation.length() < 0.001) && (isSpin ? Float.isNaN(degreesPerSecond) || degreesPerSecond == 0 : endDegrees == 0)) return false;
var axis = null;
...
nFrames = Clazz.doubleToInt(Math.abs(endDegrees) / Math.abs(degreesPerSecond) * this.spinFps + 0.5);
...
}this.setSpin(eval, true, endDegrees, finalPoints, dihedralList, vectorMatrix, bsAtoms, isGesture, centerAndPoints, centerAndPoints == null ? -1 : nFrames);
...
return false;
}, "J.api.JmolScriptEvaluator,JU.T3,JU.T3,~N,~N,~B,~B,JU.BS,~B,JU.V3,JU.Lst,~A,JU.M3,JU.M4,~B,~A");
...
function(){
this.setSpin(null, true, 3.4028235E38, null, null, null, null, false, null, -1);
});
Clazz.defineMethod(c$, "setSpinOff", 
function(){
this.setSpin(null, false, 3.4028235E38, null, null, null, null, false, null, -1);
});
Clazz.defineMethod(c$, "setSpin", 
function(eval, spinOn, endDegrees, endPositions, dihedralList, vectorMatrix, bsAtoms, isGesture, centerAndPoints, nFrames){
if (this.navOn && spinOn) this.setNavOn(false);
if (this.spinOn == spinOn) return;
this.spinOn = spinOn;
this.vectorSpinOnly = (vectorMatrix != null);
...
this.spinThread.setManager(this, this.vwr,  Clazz.newArray(-1, [Float.$valueOf(endDegrees), endPositions, dihedralList, vectorMatrix, bsAtoms, isGesture ? Boolean.TRUE : null, centerAndPoints, Integer.$valueOf(nFrames)]));
this.spinIsGesture = isGesture;
if (vectorMatrix != null || (Float.isNaN(endDegrees) || endDegrees == 3.4028235E38 || !this.vwr.g.waitForMoveTo)) {
...
this.spinThread = null;
}}, "J.api.JmolScriptEvaluator,~B,~N,JU.Lst,~A,JU.M3,JU.BS,~B,~A,~N");
Clazz.defineMethod(c$, "setNavOn", 
function(navOn){
if (Float.isNaN(this.navFps)) return;
var wasOn = this.navOn;
if (navOn && this.spinOn) this.setSpin(null, false, 0, null, null, null, null, false, null, -1);
this.navOn = navOn;
...
this.vibrationT.x = this.vibrationT.y = this.vibrationT.z = 0;
return;
}if (this.vwr.ms.mc < 1) {
this.vibrationOn = false;
this.vibrationT.x = this.vibrationT.y = this.vibrationT.z = 0;
