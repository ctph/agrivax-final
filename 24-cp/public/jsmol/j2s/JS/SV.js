...
Clazz.instantialize(this, arguments);}, JS, "SV", JS.T, javajs.api.JSONEncodable);
...
c$.copySafely = Clazz.defineMethod(c$, "copySafely", 
function(v){
...
function(x){
return (Clazz.instanceOf(x,"JS.SV") || Clazz.instanceOf(x, Boolean) || Clazz.instanceOf(x, Integer) || Clazz.instanceOf(x, Float) || (typeof(x)=='string') || Clazz.instanceOf(x,"JU.T3") || Clazz.instanceOf(x,"JU.BS") || Clazz.instanceOf(x,"JU.P4") || Clazz.instanceOf(x,"JU.Quat") || Clazz.instanceOf(x,"JU.M34") || Clazz.instanceOf(x,"java.util.Map") || Clazz.instanceOf(x,"JU.Lst") || Clazz.instanceOf(x,"JU.BArray") || Clazz.instanceOf(x,"JS.ScriptContext") || JS.SV.isArray(x));
}, "~O");
...
break;
default:
return ((Clazz.instanceOf(tokenIn,"JS.SV")) && (tokenIn).myName != null ? JS.SV.copySafely(tokenIn) : tokenIn);
}
var bs = null;
var s = null;
...
data =  Clazz.newFloatArray (len, 0);
for (var i = 0; i < len; i++) data[i] = JS.SV.fValue(sv.get(i));

break;
case 8:
var p = $var.value;
data =  Clazz.newFloatArray(-1, [p.x, p.y, p.z]);
break;
case 9:
var p4 = $var.value;
data =  Clazz.newFloatArray(-1, [p4.x, p4.y, p4.z, p4.w]);
break;
}
if (pt1 > 0) {
...
function(format){
return (format.indexOf(";") >= 0 ? -1 : ";json;base64;bytearray;array;xyz;abc;uvw;".indexOf(";" + format.toLowerCase() + ";"));
}, "~S");
...
if (pt >= 0) return JS.SV.getFormat(args[1], pt);
}
...
c$.getFormat = Clazz.defineMethod(c$, "getFormat", 
function(sv, pt){
switch (pt) {
case 0:
var name = sv.myName;
...
switch (sv.tok) {
case 15:
bytes = JU.AU.arrayCopyByte((sv.value).data, -1);
break;
case 7:
var l = sv.getList();
if (pt == 22) {
...
default:
var s = sv.asString();
if (s.startsWith(JV.JC.BASE64_TAG)) {
if (pt == 5) return s;
...
this.addToMap(m, mapKey, value);
}}return this;
}, "JS.SV,JS.SV");
Clazz.defineMethod(c$, "addToMap", 
function(m, mapKey, value){
m.put(mapKey.asString(), JS.SV.copySafely(value));
}, "java.util.Map,JS.SV,JS.SV");
...
case 10:
o2 =  new JU.Lst();
var bs = JS.SV.getBitSet(this, true);
for (var i = bs.nextSetBit(0); i >= 0; i = bs.nextSetBit(i + 1)) {
o2.addLast(JS.SV.newI(i));
}
...
case 6:
case 14:
this.getMap().put(key, JS.SV.copySafely(v));
break;
}
}, "~S,JS.SV");
