...
Clazz.load(["JU.M4", "$.P3"], "JS.SymmetryOperation", ["java.util.Hashtable", "JU.Lst", "$.Matrix", "$.Measure", "$.P4", "$.PT", "$.SB", "$.V3", "JU.BoxInfo", "$.Logger", "$.Parser"], function(){
...
var a = Math.abs(x);
var afrac = a % 1;
return (a < 0.0001 ? "" : (x < 0 ? "-" : strT.length == 0 ? "" : "+") + (a > 0.9999 && a <= 1.0001 ? "" : afrac <= 0.001 && !allowFractions ? "" + Clazz.floatToInt(a) : anyFraction ? "" + a : JS.SymmetryOperation.twelfthsOf(a * 12))) + sx;
}, "~S,~N,~S,~B,~B");
