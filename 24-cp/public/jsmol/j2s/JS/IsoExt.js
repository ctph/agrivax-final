...
Clazz.load(["JS.ScriptExt"], "JS.IsoExt", ["java.util.Hashtable", "JU.AU", "$.BS", "$.Lst", "$.M4", "$.Measure", "$.P3", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.c.VDW", "JS.SV", "$.ScriptEval", "$.ScriptInterruption", "$.T", "JU.BSUtil", "$.BoxInfo", "$.C", "$.Escape", "$.Logger", "$.Parser", "$.TempArray", "$.Triangulator", "JV.JC"], function(){
var c$ = Clazz.declareType(JS, "IsoExt", JS.ScriptExt);
...
break;
case 1073742330:
var data =  Clazz.newArray(-1, [eval.objectNameParameter(i + 3), null]);
if (this.chk) return;
...
var cpts = JU.Measure.getLatticePoints(uc.getLatticeCentering(), Clazz.floatToInt(lattice.x), Clazz.floatToInt(lattice.y), Clazz.floatToInt(lattice.z));
...
pts = this.getBoxPoints(tokIntersectBox, uc, null, null, Clazz.doubleToInt(intScale / 100));
isIntersect = true;
}}plane = null;
var linePts = null;
var isAll = false;
...
linePts =  Clazz.newArray(-1, [this.centerParameter(++i), this.centerParameter(eval.iToken + 1)]);
} else {
...
opList =  Clazz.newIntArray (params.length, 0);
for (var j = opList.length; --j >= 0; ) opList[j] = Clazz.floatToInt(params[j]);
...
if (s == null) return;
if (isSymop && Clazz.instanceOf(target,"JM.Atom") && Clazz.instanceOf(center,"JM.Atom")) {
...
propertyList.add(1,  Clazz.newArray(-1, ["ignore", JU.BS.copy(this.vwr.slm.bsDeleted)]));
...
Clazz.defineMethod(c$, "getBoxPoints", 
