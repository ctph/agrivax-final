...
Clazz.load(["JS.ScriptExt"], "JS.CmdExt", ["java.util.Hashtable", "JU.AU", "$.BS", "$.Base64", "$.Lst", "$.M3", "$.M4", "$.Measure", "$.P3", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.c.STER", "$.VDW", "J.i18n.GT", "JM.AtomCollection", "$.LabelToken", "$.Measurement", "$.Text", "JS.SV", "$.ScriptCompiler", "$.ScriptError", "$.ScriptEval", "$.ScriptInterruption", "$.ScriptMathProcessor", "$.ScriptParam", "$.T", "JU.BSUtil", "$.BoxInfo", "$.C", "$.Edge", "$.Elements", "$.Escape", "$.Logger", "$.Parser", "$.Point3fi", "$.SimpleUnitCell", "JV.FileManager", "$.JC", "$.StateManager", "$.Viewer"], function(){
var c$ = Clazz.declareType(JS, "CmdExt", JS.ScriptExt);
...
Clazz.defineMethod(c$, "audio", 
function(){
var htParams =  new java.util.Hashtable();
var id = null;
var filter = null;
var filename = null;
var i = this.e.iToken + 1;
if (this.tokAt(i) == 1073742334) {
this.checkLength(i + 1);
if (!this.chk) this.vwr.sm.stopAudio(null);
return;
}for (; i < this.slen; i++) {
switch (this.tokAt(i)) {
case 1073741974:
...
default:
this.checkLength(i + 1);
if (htParams.containsKey("action")) this.invArg();
var action = this.paramAsStr(i);
...
var isAtoms = !(Clazz.instanceOf(bs,"JM.BondSet"));
if (isAtoms) {
if (label == null) {
...
oabc = this.e.getPointArray(i, -1, false, true);
i = this.e.iToken;
...
var center = null;
if (this.tokAt(e.iToken) == 12289) {
center = e.centerParameter(e.iToken + 1, null);
}this.showString(this.vwr.ms.calculatePointGroup(bs1, center));
}}return;
case 1111490574:
this.checkLength(2);
if (!this.chk) {
...
case 1073741915:
asDSSP = true;
version = (this.slen == e.iToken + 1 ? -1 : Clazz.floatToInt(this.floatParameter(++e.iToken)));
break;
case 0:
...
var i = 1;
var tok = this.tokAt(i);
var isTransparent = (tok == 1111490588);
...
if (!looping) this.showString(J.i18n.GT.o(J.i18n.GT.$("Note: Enable looping using the LOOP keyword just after the file name or {0}"),  Clazz.newArray(-1, ["ANIMATION MODE LOOP"])));
...
vAtomSets.addLast( Clazz.newArray(-1, [bsAtoms1, (bsAtoms2 == null ? coordTo : bsAtoms2)]));
...
if (Float.isNaN(nSeconds) || nSeconds < 0) nSeconds = 1;
 else if (!doMorph && !doRotate && !doTranslate) doRotate = doTranslate = true;
...
if (isFrames) {
var oabc = this.vwr.getV0abc(iModel,  Clazz.newArray(-1, [m4]));
...
if (doMorph) {
ptsB =  new JU.Lst();
for (var n = centerAndPoints[0].length, i = 0; i < n; i++) {
...
ptsB = JS.ScriptParam.transformPoints(ptsA, m4, center);
}if (!eval.useThreads(true)) doAnimate = false;
if (this.vwr.rotateAboutPointsInternal(eval, center, pt1, endDegrees / nSeconds, endDegrees, doAnimate, bsFrom, translation, ptsB, null, null, null, false, centerAndPoints) && doAnimate && eval.isJS) throw  new JS.ScriptInterruption(eval, "compare", 1);
}
});
...
}if (Clazz.instanceOf(target,"JU.P3")) {
...
Clazz.defineMethod(c$, "showMeasureCount", 
function(isFull){
...
value =  Clazz.newFloatArray(-1, [p.x, p.y, p.z]);
} else {
...
}if (!this.chk) this.vwr.getMinimizer(true).setProperty("constraint",  Clazz.newArray(-1, [aList, Float.$valueOf(targetValue)]));
return;
case 12293:
if (i != 1) this.invArg();
if (this.slen == ++i) {
...
var list;
if (isFile) {
list =  Clazz.newArray(-1, [sequence]);
sequence = null;
} else {
...
}if (!this.chk) list.addLast( Clazz.newArray(-1, [Integer.$valueOf(268441090), Float.$valueOf(timeSec), Float.$valueOf(x), Float.$valueOf(y)]));
continue;
case 268441088:
continue;
case 1112152078:
...
if (!this.chk && !this.vwr.isJmolDataFrame() && !this.vwr.is2D()) this.vwr.tm.navigateList(eval, list);
});
...
function(args){
if (args == null && this.vwr.haveAccessInternal(null)) this.invArg();
var eval = this.e;
var pt = 1;
var pt0 = 1;
var scripts = null;
...
if (JS.CmdExt.tokAtArray(pt, args) == 2) {
quality = JS.SV.iValue(this.tokenAt(pt++, args));
}}break;
}
if (pt0 < argCount) {
...
if (!type.equals("PLB")) type = (fileName != null && fileName.indexOf(".") >= 0 ? fileName.substring(fileName.lastIndexOf(".") + 1).toUpperCase() : "JVXL");
if (type.equals("PMESH")) type = "ISOMESH";
 else if (type.equals("PMB")) type = "ISOMESHBIN";
if (type.equals("PLY")) type = "ISOPLY";
 else if (type.equals("PLB")) type = "ISOPLYBIN";
...
}if (!isImage && !isExport && !JU.PT.isOneOf(type, ";SCENE;JMOL;ZIP;ZIPALL;SPT;HISTORY;MO;NBO;ISOSURFACE;MESH;PMESH;PMB;PLY;PLB;ISOMESHBIN;ISOMESH;ISOPLY;ISOPLYBIN;VAR;FILE;FUNCTION;CFI;CIF;CIFP1;CML;JSON;XYZ;XYZRN;XYZVIB;MENU;MOL;MOL67;PDB;PGRP;PQR;PWMAT;PWSLAB;QUAT;RAMA;SDF;V2000;V3000;QCJSON;XSF;INLINE;")) eval.errorStr2(54, "COORDS|FILE|FUNCTIONS|HISTORY|IMAGE|INLINE|ISOSURFACE|JMOL|MENU|MO|NBO|POINTGROUP|QUATERNION [w,x,y,z] [derivative]|RAMACHANDRAN|SPT|STATE|VAR x|ZIP|ZIPALL|CLIPBOARD", "CIF|CIFP1|CML|CFI|GIF|GIFT|JPG|JPG64|JMOL|JVXL|MESH|MOL|PDB|PMESH|PNG|PNGJ|PNGT|PPM|PQR|PWMAT|PWSLAB|SDF|CD|JSON|QCJSON|V2000|V3000|SPT|XJVXL|XSF|XYZ|XYZRN|XYZVIB|ZIP" + driverList.toUpperCase().$replace(';', '|'));
if (this.chk) return "";
...
data = this.vwr.ms.getPointGroupAsString(this.vwr.bsA(), null, 0, 1, null, null, type2.equals("draw") ? "" : null);
...
if ((bytes = this.getIsosurfaceJvxl(24, "ISOPLYBIN")) == null) this.error(31);
type = "PLB";
...
if (bytes == null) {
var s = o.asString();
bytes = (s.startsWith(JV.JC.BASE64_TAG) ? JU.Base64.decodeBase64(s) : s.getBytes());
...
var isNames = sdiag.toLowerCase().equals("names");
var isSubgroup = sdiag.toLowerCase().startsWith("subgroups");
var isDiagram = sdiag.toLowerCase().startsWith("diagram");
var isDiagramITA = sdiag.startsWith("diagramITA");
var isTable = sdiag.toLowerCase().startsWith("table");
len = this.slen;
var ita = -1;
if (isNames) {
if (this.chk) break;
...
if (this.chk) break;
sdiag = JU.PT.trim(sdiag.substring(isDiagramITA ? 10 : isSubgroup ? 9 : isDiagram ? 7 : 5).trim(), "\"");
if (sdiag.length == 0) {
if (sym == null) {
...
ita = JU.PT.parseInt(sdiag);
if (ita == -2147483648) {
if (sym == null) {
sym = this.vwr.getSymStatic();
}var info = sym.getSpaceGroupInfo(this.vwr.ms, JU.PT.rep(sdiag, "''", "\""), -1, true, null);
if (info == null) {
msg = "Could not find space group " + sdiag;
break;
}ita = ((info.get("spaceGroupInfo")).get("ita")).intValue();
}}if (isNames || ita > 0) {
var href = (isNames ? "https://www.cryst.ehu.es/cgi-bin/cryst/programs/nph-getgen" : JV.JC.resolveDataBase(isDiagramITA ? "itadiagramita" : isDiagram ? "itadiagram" : "itatable", JU.PT.formatS("" + ita, 3, 0, false, true), null));
msg = href;
...
if (this.tokAt(eval.iToken + 1) != 0 && this.tokAt(eval.iToken + 1) != 4) {
pt2 = eval.centerParameter(++eval.iToken, ret);
if (ret[0] != null && (ret[0]).isEmpty()) {
len = this.slen;
break;
}}if (iop == 0 && this.tokAt(eval.iToken + 1) == 2) nth = eval.getToken(++eval.iToken).intValue;
}var type = (eval.iToken > 1 && this.tokAt(eval.iToken + 1) == 4 ? this.stringParameter(++eval.iToken) : pt1 == null && pt2 == null ? "label" : null);
...
msg = (o == null ? "" : Clazz.instanceOf(o,"java.util.Map") ? JS.SV.getVariable(o).asString() : o.toString());
}break;
}
break;
case 1648363544:
...
ucname = "surface" + (Clazz.floatToInt(hkl.x)) + (Clazz.floatToInt(hkl.y)) + (Clazz.floatToInt(hkl.z));
break;
case 536875059:
case 1073742163:
...
i = eval.iToken;
if (!this.chk) this.vwr.setUnitCellAtomIndex(iAtom);
if (iAtom < 0) return;
break;
case 1073742066:
isOffset = true;
...
if (sym != null && sym.getUnitCellInfoType(9) == 0) return;
s = "a-b,b-c,a+b+c";
}ucname = s;
...
oabc = (sym == null ?  Clazz.newArray(-1, [JU.P3.new3(0, 0, 0), JU.P3.new3(1, 0, 0), JU.P3.new3(0, 1, 0), JU.P3.new3(0, 0, 1)]) : sym.getUnitCellVectors());
...
if (isModelkit && this.tokAt(i + 1) == 1073742080) {
isPacked = true;
i = ++eval.iToken;
...
Clazz.defineMethod(c$, "getUVBoxFromHKL", 
function(uc, hkl, plane){
var h = Clazz.floatToInt(hkl.x);
var k = Clazz.floatToInt(hkl.y);
var l = Clazz.floatToInt(hkl.z);
...
return;
}value = (this.tokAt(i + 1) == 4 ? this.paramAsStr(++i).toLowerCase() : null);
if (value == null || !this.vwr.isModelKitOption('U', value)) {
this.unitcell(2, true);
return;
}break;
case 1088421903:
...
} else if (Clazz.instanceOf(this.getToken(i).value,"JM.BondSet")) {
...
pts = this.e.getPointArray(this.e.iToken + 1, -1, false, true);
...
var w = (wyckoff.equalsIgnoreCase("all") ? 'L' : wyckoff.equalsIgnoreCase("general") ? 'G' : (wyckoff.length > 1 || wyckoff.length == 0 ? '\0' : wyckoff.charAt(0)));
if (w < 'a' && w != 'A' && w != 'G' && w != 'L' || w > 'z') this.invArg();
wyckoff = "" + w;
...
if (this.e.isArrayParameter(i)) {
pts = this.e.getPointArray(i, -1, false, true);
} else {
pt = this.getPoint3f(i, true);
}i = this.e.iToken;
...
if (type.equals(".")) {
if (sym == null) type = "P1";
 else type = (i == this.slen ? null : sym.getSpaceGroupClegId());
...
}if (!isPacked && this.tokAt(this.e.iToken + 1) == 1073742080) {
isPacked = true;
++this.e.iToken;
}} else if (isConnect) {
if (index2 < 0) {
...
if (!isPacked) {
var ptend = i + 1;
for (var j = ptend; j < this.slen; j++) {
if (this.tokAt(j) == 1073742080) {
isPacked = true;
ptend = j;
break;
}}
if (ptend < this.slen) type = null;
}if (type == null) type = this.concatString(i, (isPacked ? "packed" : "unitcell"));
if (type.length > 0 && type.indexOf(":") < 0 && type.indexOf(">") < 0 && (type.indexOf(",") > 0 || "rh".indexOf(type) >= 0)) {
if (sym == null) this.invArg();
var cleg = sym.getSpaceGroupClegId();
type = cleg.substring(0, cleg.indexOf(":") + 1) + type;
}}var s = this.assignSpaceGroup(bs, type, paramsOrUC, isPacked, doDraw, this.e.fullCommand);
...
function(bs, cleg, paramsOrUC, isPacked, doDraw, cmd){
var sb =  new JU.SB();
var ret = this.vwr.getSymStatic().staticTransformSpaceGroup(bs, cleg, paramsOrUC, sb);
var isError = ret.endsWith("!");
if (isError) return ret;
if (isPacked || doDraw) this.vwr.getModelkit(false).cmdAssignSpaceGroup(ret, sb, cmd, isPacked, doDraw);
return sb.toString();
}, "JU.BS,~S,~O,~B,~B,~S");
Clazz.defineMethod(c$, "concatString", 
function(i, butNot){
var s = "";
for (; i < this.slen; i++) {
var t = JS.SV.sValue(this.e.getToken(i));
if (t.equals(butNot)) {
this.e.iToken--;
break;
}s += t;
...
Clazz.defineMethod(c$, "getUnitCellParameter", 
function(i, ret){
ret[0] = null;
...
ret[0] = this.e.getPointArray(i, 4, false, true);
} catch (ee) {
...
;//5.0.1-v7 Fri May 30 09:31:21 CDT 2025
