...
Clazz_load(["java.util.Hashtable", "JU.Lst"], "J.adapter.readers.pymol.PickleReader", ["java.util.Arrays", "JU.AU", "JU.Logger"], function(){
...
Clazz_defineMethod(c$, "dump", 
function(key, o){
var s = null;
if (o == null) {
s = "null";
} else if ((typeof(o)=='string') || Clazz_instanceOf(o, Number) || Clazz_instanceOf(o, Boolean)) {
s = (o === "" ? "\"\"" : o.toString());
} else if (Clazz_instanceOf(o,Array)) {
var b = o;
...
}return (color < J.adapter.readers.pymol.PyMOL.colors.length ? J.adapter.readers.pymol.PyMOL.colors[color] : color);
}, "~N");
...
return 0.25;
default:
JU.Logger.error("PyMOL " + pymolVersion + " default double setting not found: " + i);
return 0;
}
}, "~N,~N");
...
c$.fixAllZeroLabelPosition = Clazz_defineMethod(c$, "fixAllZeroLabelPosition", 
function(labelPos){
for (var i = 0; i < 7; i++) {
if (labelPos[i] != 0) return labelPos;
}
return null;
}, "~A");
...
;//5.0.1-v7 Tue May 20 13:40:34 CDT 2025
Clazz_declarePackage("J.adapter.readers.pymol");
Clazz_load(null, "J.adapter.readers.pymol.JmolObject", ["java.util.Hashtable", "JU.P3", "$.PT", "$.SB", "J.adapter.readers.pymol.PyMOLReader", "JU.BSUtil", "$.Escape"], function(){
var c$ = Clazz_decorateAsClass(function(){
this.shape = 0;
this.bsAtoms = null;
this.info = null;
...
var hsize = Clazz_floatToInt(this.size * stick_h_scale);
...
this.colors =  Clazz_newArray(-1, [colixes, Double.$valueOf(translucency)]);
}, "~A,~N");
Clazz_defineMethod(c$, "setSize", 
function(size){
this.size = Clazz_doubleToInt(size * 1000);
}, "~N");
...
this.sceneSettings =  Clazz_newDoubleArray (1000, 0);
...
if (o == null) return Clazz_doubleToInt(J.adapter.readers.pymol.PyMOL.getDefaultSetting(i, this.pymolVersion));
...
}this.objectInfo.put(this.objectName,  Clazz_newArray(-1, [this.objectStateName, Integer.$valueOf(type)]));
...
Clazz_defineMethod(c$, "getDefaultFloat", 
function(key){
return this.floatSetting(key);
}, "~N");
Clazz_defineMethod(c$, "getDefaultInt", 
function(key){
return Clazz_doubleToInt(this.getDefaultFloat(key));
}, "~N");
Clazz_defineMethod(c$, "getDefaultBoolean", 
function(key){
...
var pymolView =  Clazz_newDoubleArray (21, 0);
...
var icolor = Clazz_doubleToInt(this.getUniqueFloatDef(uniqueID, 66));
...
Clazz_defineMethod(c$, "isDefaultSettingID", 
function(id, key){
...
Clazz_defineMethod(c$, "getUniqueFloatDefVal", 
...
Clazz_defineMethod(c$, "getUniqueSetting", 
function(id, key){
...
if (labelPos == null) labelPos =  Clazz_newFloatArray (7, 0);
labelPos[0] = 1;
if (isPlacement) {
labelPos[4] = offset.x;
labelPos[5] = offset.y;
...
if (offset == null) offset = this.setLabelPosition(this.labelPosition,  Clazz_newFloatArray (7, 0), false);
...
if (offset == null) offset =  Clazz_newFloatArray(-1, [1, 0, 0, 0, 0, 0, 0]);
...
md.set(12290, null, null, null, strFormat, "angstroms", null, false, false, null, false, Clazz_doubleToInt(rad * 2000), colix, text, NaN, null);
this.addJmolObject(6, bs, md);
}
return true;
}, "~A,~N,JU.Lst,JU.BS,~N,JU.Lst,~B");
c$.newP3i = Clazz_defineMethod(c$, "newP3i", 
function(p){
...
Clazz_defineMethod(c$, "addVersionSetting", 
...
Clazz_defineMethod(c$, "addSpacefill", 
...
jo = this.addJmolObject(135180, bs,  Clazz_newArray(-1, [this.getDefaultBoolean(156) ? "FULLYLIT" : "FRONTLIT", (surfaceMode == 3 || surfaceMode == 4) ? " only" : "", this.bsCarve, Double.$valueOf(withinDistance)]));
...
info =  Clazz_newArray(-1, [bsH, Double.$valueOf(this.getUniqueFloatDef(repType, 605))]);
...
var colixes =  Clazz_newShortArray (n, 0);
...
var c = Clazz_doubleToInt(this.getUniqueFloatDefVal(id, setColor, color));
...
function(bs){
var info =  Clazz_newDoubleArray(-1, [this.floatSetting(378), this.floatSetting(377), this.floatSetting(382), this.floatSetting(379), this.floatSetting(380), this.floatSetting(381), this.floatSetting(581)]);
...
}var c = Clazz_doubleToInt(this.getUniqueFloatDefVal(id, 376, 2147483647));
...
Clazz_defineMethod(c$, "addStickBall", 
function(iatom){
...
Clazz_defineMethod(c$, "isStickBall", 
function(id){
return (this.getUniqueFloatDef(id, 276) == 1);
}, "~N");
Clazz_defineMethod(c$, "getStickBallRadius", 
function(id){
return (this.isStickBall(id) ? this.getUniqueFloatDef(id, 21) * this.getUniqueFloatDef(id, 277) : 0);
}, "~N");
Clazz_defineMethod(c$, "encodeMultipleBond", 
function(uid, isSpecial){
var scalex50 = Clazz_doubleToInt(isSpecial ? this.getUniqueFloatDef(uid, 512) * 50 : 0) & 0x3F;
return (scalex50 << 2) | 98304;
}, "~N,~B");
Clazz_defineMethod(c$, "getRadiusForBond", 
function(id, a1, a2){
...
Clazz_defineMethod(c$, "setNumAtoms", 
function(nAtomsJmol){
this.jmolToUniqueID =  Clazz_newIntArray (nAtomsJmol, 0);
}, "~N");
c$.MEAS_DIGITS =  Clazz_newIntArray(-1, [530, 531, 532]);
});
;//5.0.1-v7 Fri May 30 04:36:42 CDT 2025
Clazz_declarePackage("J.adapter.readers.pymol");
Clazz_load(["J.adapter.readers.pdb.PdbReader", "JU.BS", "$.P3"], "J.adapter.readers.pymol.PyMOLReader", ["java.util.Hashtable", "JU.AU", "$.BC", "$.CU", "$.Lst", "$.PT", "$.V3", "J.adapter.readers.pymol.PickleReader", "$.PyMOL", "$.PyMOLScene", "J.adapter.smarter.Atom", "$.Bond", "$.Structure", "J.c.STR", "JU.BSUtil", "$.BoxInfo", "$.Logger"], function(){
var c$ = Clazz_decorateAsClass(function(){
...
Clazz_instantialize(this, arguments);}, J.adapter.readers.pymol, "PyMOLReader", J.adapter.readers.pdb.PdbReader);
Clazz_prepareFields (c$, function(){
...
Clazz_superCall(this, J.adapter.readers.pymol.PyMOLReader, "finalizeSubclassReader", []);
this.asc.setTensors();
});
...
this.pymolFrame = Clazz_doubleToInt(this.pymolScene.globalSetting(194));
this.getAtomAndStateCount(names);
this.pymolScene.setStateCount(this.stateCount);
var pymolState = Clazz_doubleToInt(this.pymolScene.globalSetting(193));
...
this.pymolToAdapterAtom =  Clazz_newIntArray (nAtoms, 0);
...
Clazz_defineMethod(c$, "fillDoubleArrayFromBytes", 
function(b, array){
try {
...
var radii =  Clazz_newDoubleArray (this.$ac, 0);
...
Clazz_defineMethod(c$, "getCartoonType", 
function(iAtom){
return Clazz_floatToInt(this.atoms[iAtom].vib.y);
}, "~N");
Clazz_defineMethod(c$, "getVDW", 
function(iAtom){
...
Clazz_defineMethod(c$, "compareAtoms", 
function(iPrev, i){
