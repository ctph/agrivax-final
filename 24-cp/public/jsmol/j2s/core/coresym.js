...
function(){
this.getSymmetry().magneticMomentsToFractional(this.acr, this.asc);
});
Clazz_defineMethod(c$, "setMagneticMoments", 
function(){
return (this.asc.iSet < 0 || !this.acr.vibsFractional ? 0 : this.getBaseSymmetry().setMagneticMoments(this.asc));
});
...
function(lstSpinFrames){
return this.spaceGroup.addSpinLattice(lstSpinFrames);
}, "JU.Lst");
...
Clazz_defineMethod(c$, "transformUVW", 
function(uvw, spinFrame, spinFrameExt){
if (!spinFrame.equals(this.spinFrame)) {
this.spinFrame = spinFrame;
this.spinFrameExt = spinFrameExt;
this.spinFramePp = this.spinFrameCartXYZ = null;
}return uvw;
}, "~S,~S,~S");
Clazz_defineMethod(c$, "magneticMomentsToFractional", 
function(acr, asc){
var a = acr.unitCellParams[0];
var b = acr.unitCellParams[1];
var c = acr.unitCellParams[2];
if (this.spinFrame != null) {
var spinABC = this.setSpinFrameMatrices(acr);
a = spinABC[1].length();
b = spinABC[2].length();
c = spinABC[3].length();
this.spinFrameCartXYZ =  new JU.M4();
var v =  new JU.P3();
for (var i = 4; --i > 0; ) {
v.setT(spinABC[i]);
if (this.spinFrameRotationMatrix != null) this.spinFrameRotationMatrix.rotate(v);
this.spinFrameCartXYZ.setColumn4(i - 1, v.x, v.y, v.z, 0);
}
System.out.println("XtalSymmetry spinFramePp=" + this.spinFramePp + "\nspinFrameCartXYZ=" + this.spinFrameCartXYZ);
...
}, "J.adapter.smarter.AtomSetCollectionReader,J.adapter.smarter.AtomSetCollection");
Clazz_defineMethod(c$, "setSpinFrameMatrices", 
function(acr){
System.out.println("XtalSymmetry.setSpinFrameMatrices using frame " + this.spinFrame);
this.setUnitCellFromParams(acr.unitCellParams, false, NaN);
this.spinFramePp = this.convertTransform(this.spinFrame, null);
this.spinFramePp =  new JU.M4();
var spinABC = JS.UnitCell.getMatrixAndUnitCell(acr.vwr, this.unitCell, this.spinFrame, this.spinFramePp);
var strAxis = this.getSpinExt(this.spinFrameExt, "axisxyz");
if (strAxis != null) {
var angle = JU.PT.parseFloat(this.getSpinExt(this.spinFrameExt, "angle"));
if (!Float.isNaN(angle)) {
var abc;
if (strAxis.indexOf("n") >= 0) {
abc = this.getVariableAxis(strAxis);
} else {
abc = this.getCif2Array(strAxis,  Clazz_newFloatArray (3, 0));
}if (abc != null) {
var axis = JU.V3.new3(abc[0], abc[1], abc[2]);
this.unitCell.toCartesian(axis, true);
axis.normalize();
...
Clazz_defineMethod(c$, "getCif2Array", 
function(strAxis, abc){
var v = strAxis.$plit(",");
if (v.length == abc.length) {
for (var i = abc.length; --i >= 0; ) {
...
v = v.clone();
if (this.spinFrameCartXYZ != null) {
this.spinFrameCartXYZ.rotate(v);
} else {
...
Clazz_load(["J.api.SymmetryInterface"], "JS.Symmetry", ["java.util.Hashtable", "$.Stack", "JU.AU", "$.BS", "$.JSJSONParser", "$.Lst", "$.M4", "$.P3", "$.PT", "$.Quat", "$.Rdr", "$.SB", "J.api.Interface", "J.bspt.Bspt", "JS.PointGroup", "$.SpaceGroup", "$.SymmetryInfo", "$.SymmetryOperation", "$.UnitCell", "JU.BSUtil", "$.Escape", "$.Logger", "$.SimpleUnitCell", "JV.FileManager"], function(){
var c$ = Clazz_decorateAsClass(function(){
...
for (var i = 0; i < n; i++) list[i] = "" + this.getSpaceGroupXyz(i, doNormalize);
...
var s =  new JS.Symmetry().setViewer(this.vwr);
s.unitCell = uc;
return s;
});
...
if (cellParams != null) {
cellInfo =  new JS.Symmetry().setViewer(this.vwr).setUnitCellFromParams(cellParams, false, NaN);
...
var ops = this.getSymmetryOperations();
return (ops == null || this.unitCell == null ? null : this.unitCell.getEquivPoints(pt, flags, ops, pts == null ?  new JU.Lst() : pts, 0, 0, 0, this.getPeriodicity()));
}, "JU.Lst,JU.P3,~S");
...
this.unitCell.getEquivPoints(pts.get(i), flags, ops, pts, check0, n0, dup0, per);
}
...
sg = JS.SpaceGroup.getSpaceGroupFromJmolClegOrITA(vwr, id);
...
var w = this.getWyckoffFinder().getWyckoffFinder(vwr, sg);
...
bsNew.set(group);
stack.push( Clazz_newIntArray(-1, [itaFrom, index]));
...
return null;
}, "~N,~N,~N,~N,~N,~N,~A,~N,~N,~N,JU.BS,java.util.Stack,JU.Lst");
...
var resource = JS.Symmetry.getITJSONResource(this.vwr, specialType, itno, data);
if (resource != null) {
...
var data = JS.Symmetry.getAllITAData(vwr, type, false);
if (itindex > 0) return (data[itno - 1].get("its")).get(itindex - 1);
...
function(aa){
this.unitCell.setSpinAxisAngle(aa);
}, "JU.A4");
c$.nullDesc = null;
...
;//5.0.1-v7 Fri May 30 08:11:14 CDT 2025
Clazz_declarePackage("JS");
...
Clazz_declarePackage("JS");
Clazz_load(["JS.HallInfo", "java.util.Hashtable"], "JS.SpaceGroup", ["java.util.Arrays", "JU.AU", "$.Lst", "$.M4", "$.P3", "$.PT", "$.SB", "JS.Symmetry", "$.SymmetryOperation", "$.UnitCell", "JU.Logger", "$.SimpleUnitCell"], function(){
var c$ = Clazz_decorateAsClass(function(){
this.specialPrefix = "";
this.groupType = 0;
...
if (xyz0.equals("x,y,z,m") || xyz0.equals("x,y,z(mx,my,mz)") || xyz0.equals("x,y,z(u,v,w)")) {
this.xyzList.clear();
this.operationCount = 0;
...
Clazz_defineMethod(c$, "addSpinLattice", 
function(lstSpinFrames){
if (this.latticeOp >= 0 || lstSpinFrames.size() == 0) return false;
...
if (specialType > 0) {
return JS.Symmetry.getSGFactory().getSpecialGroup(null, vwr, name, specialType);
...
;//5.0.1-v7 Fri May 30 09:14:38 CDT 2025
Clazz_declarePackage("JS");
Clazz_load(["JU.P3"], "JS.SpaceGroupFinder", ["java.util.Arrays", "JU.BS", "$.Lst", "$.M4", "$.Measure", "$.PT", "$.V3", "JS.SpaceGroup", "$.Symmetry", "$.SymmetryOperation", "JU.BSUtil", "JV.FileManager"], function(){
var c$ = Clazz_decorateAsClass(function(){
this.vwr = null;
this.uc = null;
this.sg = null;
...
var sym =  new JS.Symmetry().setViewer(this.vwr);
...
(uc =  new JS.Symmetry()).setViewer(vwr).getUnitCell(oabc, false, "scaled");
var f = 0;
for (var i = bsPoints.nextSetBit(0); i >= 0; i = bsPoints.nextSetBit(i + 1)) {
switch (abc) {
case 1:
...
;//5.0.1-v7 Fri May 30 04:18:34 CDT 2025
Clazz_declarePackage("JS");
...
;//5.0.1-v7 Fri May 30 04:18:34 CDT 2025
Clazz_declarePackage("JS");
Clazz_load(["JU.M4", "$.P3"], "JS.SymmetryOperation", ["java.util.Hashtable", "JU.Lst", "$.Matrix", "$.Measure", "$.P4", "$.PT", "$.SB", "$.V3", "JU.BoxInfo", "$.Logger", "$.Parser"], function(){
var c$ = Clazz_decorateAsClass(function(){
...
xyz = xyz.substring(0, pt);
var v =  Clazz_newFloatArray (16, 0);
JS.SymmetryOperation.getRotTransArrayAndXYZ(null, suvw, v, true, false, false, "uvw");
this.spinU = JU.M4.newA16(v);
...
function(strT, x, sx, allowFractions, anyFraction){
var a = Math.abs(x);
var afrac = a % 1;
return (a < 0.0001 ? "" : (x < 0 ? "-" : strT.length == 0 ? "" : "+") + (a > 0.9999 && a <= 1.0001 ? "" : afrac <= 0.001 && !allowFractions ? "" + Clazz_floatToInt(a) : anyFraction ? "" + a : JS.SymmetryOperation.twelfthsOf(a * 12))) + sx;
}, "~S,~N,~S,~B,~B");
...
function(mat, is12ths, allPositive, halfOrLess, allowFractions, anyFraction, labels){
var str = "";
...
term += JS.SymmetryOperation.plusMinus(term, x, labels_[j + lpt], allowFractions, anyFraction);
}}
...
function(m, xyz){
if (this.spinU != null) return xyz + JS.SymmetryOperation.getSpinString(this.spinU, true);
...
return xyz + JS.SymmetryOperation.getSpinString(m2, false);
}, "JU.M4,~S");
c$.getSpinString = Clazz_defineMethod(c$, "getSpinString", 
function(m, isUVW){
return "(" + JS.SymmetryOperation.getXYZFromMatrixFrac(m, false, false, false, isUVW, isUVW, (isUVW ? "uvw" : "mxyz")) + ")";
}, "JU.M4,~B");
Clazz_defineMethod(c$, "getInfo", 
function(){
if (this.info == null) {
...
if (this.spinU != null) this.info.put("spinU", this.spinU);
...
t.setColumnA(3, v);
}return JS.SymmetryOperation.getXYZFromMatrixFrac(t, false, true, false, allowFractions, false, null);
...
m.add(tr);
m.transpose();
var s = JS.SymmetryOperation.getXYZFromMatrixFrac(m, false, true, false, true, false, null).$replace('x', 'a').$replace('y', 'b').$replace('z', 'c');
...
function(xyz, matrix, labels){
var toMat = (matrix == null);
if (toMat) {
matrix = JS.SymmetryOperation.stringToMatrix(xyz, labels);
}if ("rxyz".equals(labels)) {
return JS.SymmetryOperation.matrixToRationalString(matrix);
}return (toMat ? matrix : JS.SymmetryOperation.getXYZFromMatrixFrac(matrix, false, false, false, true, false, labels));
}, "~S,JU.M4,~S");
c$.atomTest = null;
...
;//5.0.1-v7 Fri May 30 08:56:14 CDT 2025
Clazz_declarePackage("JS");
...
var shiftA = (isSpaceGroup && (periodicity & 0x1) == 0);
var shiftB = (isSpaceGroup && (periodicity & 0x2) == 0);
var shiftC = (isSpaceGroup && (periodicity & 0x4) == 0);
...
;//5.0.1-v7 Fri May 30 08:07:30 CDT 2025
Clazz_declarePackage("JS");
...
v.normalize();
var s = "rotation_axis=" + (v.x == Math.round(v.x) ? "" + Math.round(v.x) : this.round000(v.x)) + "," + (v.y == Math.round(v.y) ? "" + Math.round(v.y) : this.round000(v.y)) + "," + (v.z == Math.round(v.z) ? "" + Math.round(v.z) : this.round000(v.z));
if (ptAxis < 0) {
this.moreInfo.addLast(s);
if (ptAngle > ptAxis) ptAngle++;
} else {
this.moreInfo.set(ptAxis, s);
}var a = (aa.angle * 180 / 3.141592653589793);
a = Clazz_doubleToInt(Math.round(a * 1000) / 1000);
s = "rotation_angle=" + (a == Math.round(a) ? "" + Math.round(a) : this.round000(a));
if (ptAngle < 0) {
this.moreInfo.addLast(s);
} else {
this.moreInfo.set(ptAngle, s);
}}, "JU.A4");
Clazz_defineMethod(c$, "round000", 
function(y){
...
;//5.0.1-v7 Fri May 30 04:24:53 CDT 2025
Clazz_declarePackage("JS");
...
/*LV!1824 unnec constructor*/c$.standardizeTokens = Clazz_defineMethod(c$, "standardizeTokens", 
...
Clazz_defineMethod(c$, "transformSpaceGroup", 
