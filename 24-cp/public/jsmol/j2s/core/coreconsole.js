...
Clazz_instantialize(this, arguments);}, J.console, "GenericConsole", null, [J.api.JmolAppConsoleInterface, J.api.JmolCallbackListener]);
...
var strErrorMessage = this.vwr.script(cmd + "; ## GUI ##\u0001## EDITOR_IGNORE ##; ## GUI ##");
...
if (mystatus.indexOf("Picked") >= 0 || mystatus.indexOf("Sequence") >= 0) this.sendConsoleMessage(strInfo);
 else if (mystatus.indexOf("Completed") >= 0) this.sendConsoleEcho(strInfo);
break;
case J.c.CBK.MESSAGE:
...
function(up, findLastLikeThis){
var cmd = (findLastLikeThis ? this.vwr.historyFind(this.pageUpBuffer == null ? (this.pageUpBuffer = this.getCommandString()) : this.pageUpBuffer, up ? -1 : 1) : this.vwr.getSetHistory(up ? -1 : 1));
if (cmd != null) {
cmd = this.trimGUI(cmd);
this.input.setText(JU.PT.escUnicode(cmd));
}}, "~B,~B");
Clazz_defineMethod(c$, "getCommandString", 
function(){
...
this.recallCommand(kcode == 38, isControlDown);
return mode;
}break;
case 402:
...
switch ((ch = cmd.charAt(i)).charCodeAt(0)) {
case 34:
...
var c$ = Clazz_decorateAsClass(function(){
this.jsConsole = null;
Clazz_instantialize(this, arguments);}, J.consolejs, "AppletConsole", J.console.GenericConsole);
...
Clazz_overrideMethod(c$, "regainFocus", 
function(){
});
{
{
...
s = '<div spellcheck="false" id="$ID" class="jmolConsole" style="display:block;background-color:yellow;width:'+w+'px;height:'+h+'px;position:absolute;z-index:'
