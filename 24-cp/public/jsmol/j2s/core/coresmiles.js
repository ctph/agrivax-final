...
var search = JS.SmilesParser.newSearch("/nostereo/" + pattern.toString(), isSmarts, true);
...
var isWild = false;
if ((typeof(smiles1)=='string') && (typeof(smiles2)=='string')) {
isWild = ((smiles1).indexOf("*") >= 0);
if (!isWild && smiles1.equals(smiles2)) return 1;
}var flags = (isWild ? 2 : 1) | 8;
var result = this.matchPriv(smiles1, null, 0, null, null, false, flags, 2, this.newSearch(smiles2));
...
this.clearExceptions();
if ((typeof(target)=='string')) target = JS.SmilesParser.cleanPattern(target);
if ((typeof(pattern)=='string')) pattern = JS.SmilesParser.cleanPattern(pattern);
var search = this.newSearch(target);
...
Clazz_defineMethod(c$, "newSearch", 
function(s){
return (s == null ? null : Clazz_instanceOf(s,"JS.SmilesSearch") ? s : JS.SmilesParser.newSearch(JS.SmilesParser.cleanPattern(s.toString()), false, true));
}, "~O");
...
return this.matchPriv(pattern, atoms, ac, bsSelected, null, true, flags | ((typeof(pattern)=='string') ? JS.SmilesParser.getFlags(pattern.toString()) : 0), 1, (atoms == null ? target : null));
}, "~O,~O,~N,JU.BS,~N");
...
var searchPattern = (isCompiled ? pattern : JS.SmilesParser.newSearch(pattern == null ? null : pattern.toString(), isSmarts, false));
if (searchTarget != null) searchTarget.setFlags(flags | searchTarget.flags | (isCompiled ? 0 : JS.SmilesParser.getFlags(pattern.toString())));
...
if ((typeof(pattern)=='string')) pattern = JS.SmilesParser.cleanPattern(pattern);
try {
var search = ((typeof(pattern)=='string') ? JS.SmilesParser.newSearch(pattern, true, false) : pattern);
for (var i = 0; i < smilesSet.length; i++) {
var smiles = smilesSet[i];
var searchTarget;
if ((typeof(smiles)=='string')) {
searchTarget = this.newSearch(smiles);
searchTarget.setFlags(searchTarget.flags | ((typeof(pattern)=='string') ? JS.SmilesParser.getFlags(pattern) : 0));
} else {
searchTarget = smiles;
}try {
this.clearExceptions();
...
function(ss){
return (Clazz_instanceOf(ss,"JS.SmilesSearch")) && (this.pattern0 == null ? (ss).pattern0 == null : this.pattern0.equals((ss).pattern0));
}, "~O");
/*if3*/;(function(){
var c$ = Clazz_decorateAsClass(function(){
...
var c$ = Clazz_decorateAsClass(function(){
this.MODE_COMP_ALLOW_BIO = 1;
this.MODE_COMP_ALLOW_OUTSIDE = 2;
this.MODE_COMP_FORCE_BRACKETS = 4;
this.ptAtom = 0;
this.ptSp2Atom0 = 0;
this.atoms = null;
this.ac = 0;
...
this.allowBioResidues = false;
this.allowConnectionsToOutsideWorld = false;
...
return this.getSmilesComponent(atoms[ipt], bsSelected, 1);
...
s = this.getSmilesComponent(a, bs, 6);
...
function(atom, bs, mode){
var ret = "";
this.allowBioResidues = ((mode & 1) != 0);
this.allowConnectionsToOutsideWorld = ((mode & 2) != 0);
this.forceBrackets = ((mode & 4) != 0);
while (true) {
if (atom == null) atom = this.atoms[bs.nextSetBit(0)];
atom = this.checkFirstAtom(atom);
this.bsSelected = JU.JmolMolecule.getBranchBitSet(this.atoms, atom.getIndex(), JU.BSUtil.copy(bs), null, -1, true, this.allowBioResidues);
...
this.getSmilesAt(sb, this.atoms[i], false, false);
atom = null;
}}
if (atom != null) {
while ((atom = this.getSmilesAt(sb, atom, true, false)) != null) {
}
...
function(sb, atom, allowBranches, isBranch){
...
if (isH || !this.allowConnectionsToOutsideWorld || !this.bsSelected.get(atomIndex)) continue;
...
this.getSmilesAt(s2, a, allowBranches, true);
bondNext = bond0t;
this.ptAtom = ptAtomt;
...
sb.append(JS.SmilesAtom.getAtomLabel(atomicNumber, isotope, (this.forceBrackets ? -1 : valence), charge, osclass, nH, isAromatic, atat != null ? atat : this.noStereo ? null : this.checkStereoPairs(atom, this.alleneStereo == null ? atomIndex : -1, stereo, stereoFlag, prevIndex == -1), this.is2D));
}sb.appendSB(sbRings);
if (bondNext != null) {
...
function(){
var s2 = "";
for (var i = 0; i < this.bondCount; i++) s2 += this.bonds[i].getOtherAtom(this).getAtomForToString() + ";bo=" + this.bonds[i].getCovalentOrder() + ", ";

return this.getAtomForToString() + "->" + s2 + "(" + this.x + "," + this.y + "," + this.z + ")";
});
Clazz_defineMethod(c$, "getAtomForToString", 
function(){
...
function(patternObj, smilesObj, bsSelected, bsMatch3D, flags, asOneBitset, firstMatchOnly){
var info = ((typeof(patternObj)=='string') ? patternObj : "-");
if ((info.length == 0 || info.endsWith("///") || info.equals("H") || info.equals("H2") || info.equals("top") || info.equalsIgnoreCase("NOAROMATIC"))) {
try {
return this.e.vwr.getSmilesOpt(bsSelected, 0, 0, flags | (info.equals("H2") ? 8192 : 0) | (info.equals("H") ? 4096 : 0) | (info.equals("top") ? 16384 : 0) | (info.equalsIgnoreCase("NOAROMATIC") ? 16 : 0), (info.endsWith("///") ? info : null));
} catch (ex) {
if (Clazz_exceptionOf(ex, Exception)){
this.e.evalError(ex.getMessage(), null);
} else {
...
try {
if (smilesObj == null) {
b = this.e.vwr.getSubstructureSetArray(patternObj, bsSelected, flags);
} else if ("chirality".equals(info)) {
return this.e.vwr.calculateChiralityForSmiles(smilesObj.toString());
} else {
var isSmarts = ((flags & 2) == 2);
var ignoreElements = ((flags & 16384) == 16384);
flags = (isSmarts ? 2 : 1) | (firstMatchOnly ? 8 : 0) | (ignoreElements ? 16384 : 0);
if (Clazz_instanceOf(smilesObj,Array)) {
return this.e.vwr.getSmilesMatcher().hasStructure(info, smilesObj, flags);
}var map = this.e.vwr.getSmilesMatcher().find(patternObj, smilesObj, flags);
...
var stddev = this.getSmilesCorrelation(bsMatch3D, bsSelected, info, null, null, null, vReturn, false, null, null, false, flags);
