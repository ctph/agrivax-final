...
this.barPixelsXYZ = 0;
this.sppa = 0;
Clazz_instantialize(this, arguments);}, JM, "Text", null);
...
if (this.fontScale != 0 && this.font != null) this.setFont(this.vwr.gdata.getFont3DScaled(this.font, scale), true);
}, "~N");
...
if (this.font.fontSizeAngstroms != 0 || scalePixelsPerMicron != 0 && this.scalePixelsPerMicron != 0) {
if (this.font.fontSizeAngstroms == 0) {
...
boxXY[1] = this.movableY - this.yAdj + Clazz_doubleToInt((this.descent - this.ascent) / 2) + 8;
isAbsolute = true;
this.boxYoff2 = -2;
} else {
this.boxYoff2 = 0;
...
function(x, width, ppa){
var f = (x < -1 ? -1 : x > 1 ? 0 : (x - 1) / 2);
var offset = (x < -1 || x > 1 ? x + (x < 0 ? 1 : -1) : 0);
...
xLeft = 5 * scale;
xCenter = Clazz_doubleToInt((this.windowWidth + this.barPixels) / 2);
...
function(atomPt, screen, zSlab, pTemp, sppa){
this.sppa = sppa;
var isPixel = this.isPymolOffsetPixels();
var isRelative = this.isPymolOffsetRelative();
...
Clazz_defineMethod(c$, "isPymolOffsetRelative", 
function(){
var mode = (this.pymolOffset == null ? -1 : Clazz_floatToInt(this.pymolOffset[0]));
return ((mode & 1) == 1);
});
Clazz_defineMethod(c$, "isPymolOffsetPixels", 
function(){
var mode = (this.pymolOffset == null ? -1 : Clazz_floatToInt(this.pymolOffset[0]));
return (mode != -1 && ((mode & 2) == 2));
});
Clazz_defineMethod(c$, "getStateText", 
function(){
return (this.doFormatText ? JU.PT.rep(this.text, "@{", "\\@{") : this.text);
});
...
this.isHover = false;
this.isAll = false;
Clazz_instantialize(this, arguments);}, J.shape, "TextShape", J.shape.Shape);
...
if (Clazz_instanceOf(value,"JM.Text")) {
...
function(bsModels){
if (!this.isHover) for (var t, $t = this.objects.values().iterator (); $t.hasNext()&& ((t = $t.next ()) || true);) t.visible = (t.modelIndex < 0 || bsModels.get(t.modelIndex) && (!t.thisModelOnly || bsModels.cardinality() == 1 && t.modelIndex == this.vwr.am.cmi));
...
this.pickedX = 0;
this.pickedY = 0;
Clazz_instantialize(this, arguments);}, J.shape, "Labels", J.shape.AtomShape);
Clazz_prepareFields (c$, function(){
...
var n = Math.min(ac, this.strings.length);
...
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) this.setLabel(tokens, strLabel, i, !isPicked);

}if (this.vwr.getModelkitPropertySafely(null) === Boolean.TRUE) {
this.vwr.shm.notifyAtoms("atomslabeled",  Clazz_newArray(-1, [bs, null]));
}return;
...
this.isActive = true;
if (this.bsBgColixSet == null) this.bsBgColixSet = JU.BS.newN(ac);
...
}if (this.bsFontSet == null) this.bsFontSet = JU.BS.newN(ac);
...
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) {
...
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) this.setFont(i, fid);
...
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) this.setOffsets(i, offset);

}} else if (!this.setDefaults) {
this.checkColixLength(-1, ac);
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) this.setPymolOffset(i, value);
...
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) this.setHorizAlignment(i, hAlignment);
...
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) this.setPointer(i, pointer);
...
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) this.setZPos(i, 32, TF);
...
for (var i = bs.nextSetBit(0); i >= 0 && i < ac; i = bs.nextSetBit(i + 1)) this.setZPos(i, 16, TF);

}return;
}if ("display" === propertyName || "toggleLabel" === propertyName) {
var mode = ("toggleLabel" === propertyName ? 0 : (value).booleanValue() ? 1 : -1);
if (this.mads == null) this.mads =  Clazz_newShortArray (ac, 0);
...
this.setScaling();
var n = this.checkStringLength(this.ms.ac);
...
var a = this.ms.at[i];
text = JM.Text.newLabel(this.vwr, JU.Font.getFont3D(fid), this.strings[i], this.getColix2(i, a, false), this.getColix2(i, a, true), 0, this.scalePixelsPerMicron);
...
function(){
this.isActive = true;
if (this.bsSizeSet == null) this.bsSizeSet = JU.BS.newN(this.ms.ac);
...
this.offsets = JU.AU.ensureLengthI(this.offsets, this.ms.ac);
...
this.fids = JU.AU.ensureLengthI(this.fids, this.ms.ac);
}this.fids[i] = fid;
...
if (!atoms[entry.getKey().intValue()].isVisible(this.vf | 9)) continue;
...
this.htMin = null;
this.tokAction = 0;
Clazz_instantialize(this, arguments);}, J.shape, "Measures", J.shape.AtomShape, J.api.JmolMeasurementClient);
Clazz_prepareFields (c$, function(){
this.measurements =  new JU.Lst();
});
Clazz_defineMethod(c$, "getSelected", 
function(){
return this.bsSelected;
});
Clazz_overrideMethod(c$, "initModelSet", 
function(){
...
Clazz_overrideMethod(c$, "setProperty", 
function(propertyName, value, bsAtoms){
var mt;
...
this.bsSelected = JU.BSUtil.newBitSet2(0, this.measurementCount);
return;
...
if (md.bsSelected != null) this.bsSelected = md.bsSelected;
...
this.clear();
return;
}if ("atomsDeleted" === propertyName) {
for (var i = this.measurementCount; --i >= 0; ) {
out : for (var iatom = bsAtoms.nextSetBit(0); iatom >= 0; iatom = bsAtoms.nextSetBit(iatom + 1)) {
...
this.reformatDistances(value == null);
return;
...
if ((typeof(value)=='string')) {
this.doAction(null, value, 268438018);
} else {
...
if ("selected".equals(property)) return (this.bsSelected == null ?  new JU.BS() : JU.BSUtil.copy(this.bsSelected));
return null;
}, "~S,~N");
...
var isDefault = "default".equals(format);
if (isDefault || JM.Measurement.isUnits(format)) {
if (isDefault) format = null;
for (var i = this.measurements.size(); --i >= 0; ) if (this.bsSelected == null || this.bsSelected.get(i)) {
var m = this.measurements.get(i);
m.units = format;
...
this.define(( new JM.MeasurementData().init(null, this.vwr, points)).set(this.tokAction, this.htMin, this.radiusData, m.property, this.strFormat, null, this.tickInfo, this.mustBeConnected, this.mustNotBeConnected, this.intramolecular, true, 0, 0, null, NaN, null), (isDelete ? 12291 : 12290));
...
for (var i = this.measurementCount; --i >= 0; ) if (md.bsSelected.get(i)) this.processNextMeasure(md, this.measurements.get(i));

return;
}if (tokAction == 1275082241) this.bsSelected =  new JU.BS();
md.define(this, this.ms);
}, "JM.MeasurementData,~N");
Clazz_overrideMethod(c$, "processNextMeasure", 
function(md, m){
var iThis = this.find(m);
if (md.tokAction == 1275082241) {
if (iThis < 0 || !this.isInRange(NaN, m)) return;
this.bsSelected.set(iThis);
}if (iThis >= 0) {
if (this.tokAction == 12291) {
if (this.isInRange(NaN, m)) this.deleteI(iThis);
} else if (md.tokAction == 1275082241) {
var mThis = this.measurements.get(iThis);
mThis.setFromMD(md, true);
if (this.strFormat != null && md.strFormat == null) {
mThis.formatMeasurementAs(this.strFormat, md.units, true);
...
}} else if (this.tokAction == 12290 || this.tokAction == 268438018) {
...
function(val, m){
return !(this.htMin != null && !m.isMin(this.htMin) || this.radiusData != null && !m.isInRange(this.radiusData, val == val ? val : m.getMeasurement(null)));
}, "~N,JM.Measurement");
Clazz_defineMethod(c$, "defineMeasurement", 
function(i, m, doSelect){
var value = m.getMeasurement(null);
if (!this.isInRange(value, m)) return;
...
function(isDefault){
for (var i = this.measurementCount; --i >= 0; ) this.measurements.get(i).reformatDistanceIfSelected(isDefault);
...
Clazz_instantialize(this, arguments);}, J.shape, "Echo", J.shape.TextShape);
Clazz_overrideMethod(c$, "initShape", 
function(){
...
this.isAll = false;
var haveScale = (this.scaleObject != null);
var isScale = (this.thisID === "%SCALE");
var text = (isScale ? this.scaleObject : this.objects.get(target));
...
Clazz_instantialize(this, arguments);}, J.shape, "Hover", J.shape.TextShape);
Clazz_overrideMethod(c$, "initShape", 
function(){
this.isHover = true;
...
if (text.image == null) {
if (text.bgcolix != 0) {
if (g3d.setC(text.bgcolix)) J.render.TextRenderer.showBox(g3d, text.colix, Clazz_floatToInt(text.boxX) - (barPixels == 0 ? 0 : barPixels + 4), Clazz_floatToInt(text.boxY) + text.boxYoff2 * 2, text.z + 2, text.zSlab, Clazz_floatToInt(text.boxWidth) + barPixels, Clazz_floatToInt(text.boxHeight), text.fontScale, !text.isEcho);
...
c$.calcBarPixelsXYZ = Clazz_defineMethod(c$, "calcBarPixelsXYZ", 
...
barPixels = t.barPixelsXYZ = Clazz_floatToInt(barPixels * t.barDistance / d);
}}return barPixels;
}, "JV.Viewer,JM.Text,JU.P3i,~B");
c$.renderScale = Clazz_defineMethod(c$, "renderScale", 
function(g3d, text, temp, barPixels, isAntialiased){
var z = text.z;
var xoff = (text.xyz == null ? 0 : 2);
var ia = (isAntialiased ? 2 : 1);
var i = 1;
var x1 = xoff + Clazz_floatToInt(temp[0]) - barPixels - i - ia * 2;
var x2 = xoff + Clazz_floatToInt(temp[0]) - i - ia * 2;
var h = Clazz_doubleToInt((text.lineHeight) / 2);
var y = Clazz_floatToInt(temp[1]) - i;
g3d.fillTextRect(x1, y - Clazz_doubleToInt(h / 2) - ia, z, text.zSlab, x2 - x1, 2 * ia);
g3d.fillTextRect(x1, y - Clazz_doubleToInt(h * 2 / 2), z, text.zSlab, 2 * ia, Clazz_doubleToInt(h * 2 / 2));
g3d.fillTextRect(x2, y - Clazz_doubleToInt(h * 2 / 2), z, text.zSlab, 2 * ia, Clazz_doubleToInt(h * 2 / 2));
for (var j = 1; j < 10; j++) {
var x1b = x1 + Clazz_doubleToInt(j * barPixels / 10);
var len = (j == 5 ? h : Clazz_doubleToInt(h / 2));
g3d.fillTextRect(x1b, y - len, z, text.zSlab, 2 * ia, len);
}
}, "J.api.JmolRendererInterface,JM.Text,~A,~N,~B");
c$.drawLineXYZ = Clazz_defineMethod(c$, "drawLineXYZ", 
...
if (pointerWidth > 1) {
g3d.fillCylinderXYZ(pointerColix, pointerColix, 2, pointerWidth, x0, y0, z0, Clazz_floatToInt(x1), Clazz_floatToInt(y1), z1);
} else {
g3d.setC(pointerColix);
g3d.drawLineXYZ(x0, y0, z0, Clazz_floatToInt(x1), Clazz_floatToInt(y1), z1);
...
J.render.TextRenderer.showBox(g3d, colix, Clazz_floatToInt(x), Clazz_floatToInt(y), z, zSlab, Clazz_floatToInt(w), Clazz_floatToInt(h), 1, true);
} else {
g3d.setC(colix);
...
this.scalePixelsPerMicron = 0;
this.mode = 0;
Clazz_instantialize(this, arguments);}, J.render, "LabelsRenderer", J.render.FontLineShapeRenderer);
Clazz_prepareFields (c$, function(){
...
Clazz_defineMethod(c$, "setRenderVars", 
function(){
...
Clazz_defineMethod(c$, "renderText", 
...
this.aaT = null;
this.matrixT = null;
Clazz_instantialize(this, arguments);}, J.render, "MeasuresRenderer", J.render.LabelsRenderer);
...
measures.clear();
return false;
}this.doJustify = this.vwr.getBoolean(603979869);
...
this.dashDots = (this.m.text != null && this.m.text.pymolOffset != null ? J.render.FontLineShapeRenderer.pymoldashes : this.mad < 0 ? null : J.render.FontLineShapeRenderer.ndots);
}switch (this.count) {
case 1:
...
var zLabel = (this.m.inFront ? Math.min(zA, zB) - 10 : Clazz_doubleToInt((zA + zB) / 2));
...
if (s == null) return;
var zLabel = (this.m.inFront ? Math.min(Math.min(Math.min(zA, zB), zC), zD) : Clazz_doubleToInt((zA + zB + zC + zD) / 4));
if (zLabel < 1) zLabel = 1;
radius /= 3;
if (this.m.text == null) {
this.g3d.setC(this.labelColix);
...
if (this.dotsOrDashes && (this.dashDots == null || this.dashDots === J.render.FontLineShapeRenderer.ndots || this.dashDots === J.render.FontLineShapeRenderer.pymoldashes)) this.width = diameter;
...
Clazz_instantialize(this, arguments);}, J.render, "EchoRenderer", J.render.LabelsRenderer);
Clazz_overrideMethod(c$, "render", 
function(){
...
Clazz_defineMethod(c$, "renderEcho", 
function(t, alias){
...
if (t.pymolOffset != null) t.getPymolScreenOffset(t.xyz, this.pt0i, this.zSlab, this.pTemp, this.sppa);
...
}if (J.render.TextRenderer.render(this.vwr, t, this.g3d, this.sppa, this.imageFontScaling, null, this.xy, this.pt2i, 0, 0, alias) && t.valign == 1 && t.align == 12) this.vwr.noFrankEcho = false;
if (JU.C.renderPass2(t.bgcolix) || JU.C.renderPass2(t.colix)) this.haveTranslucent = true;
}, "JM.Text,~N");
Clazz_defineMethod(c$, "renderFrameTitle", 
function(frameTitle){
this.vwr.gdata.setFontBold("arial", Clazz_floatToInt(24 * this.imageFontScaling));
...
this.tempXY = null;
this.ptTemp = null;
Clazz_instantialize(this, arguments);}, J.render, "HoverRenderer", J.render.ShapeRenderer);
Clazz_prepareFields (c$, function(){
